...
// Compile, mirroring step_test.go's style): a "Score" projection
...
func TestExec_WorkflowStartedActivatesWorkflowAndStateLevelPresentations(t *testing.T) {
	...
	commit, err := Step(p, snap, engine.Signal{Name: "WorkflowStarted"}, engine.DefaultLimits())
	if err != nil {
...
func TestExec_StateChangeRemovesStateLevelKeepsWorkflowLevel(t *testing.T) {
	p := presentationProgram()
	commit, _ := Step(p, presentationSnapshot(), engine.Signal{Name: "WorkflowStarted"}, engine.DefaultLimits())
	snap := commit.Snapshot

	commit, err := Step(p, snap, engine.Signal{Name: "GoToPlaying"}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestExec_ProjectionUpdateReflectsNewGlobalState(t *testing.T) {
	p := presentationProgram()
	commit, _ := Step(p, presentationSnapshot(), engine.Signal{Name: "WorkflowStarted"}, engine.DefaultLimits())
	commit, _ = Step(p, commit.Snapshot, engine.Signal{Name: "GoToPlaying"}, engine.DefaultLimits())
	snap := commit.Snapshot

	commit, err := Step(p, snap, engine.Signal{Name: "Inc"}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	updates := 0
...
func TestExec_DuplicateTargetInOneTargetsListIsRejected(t *testing.T) {
	...
	_, err := Step(p, presentationSnapshot(), engine.Signal{Name: "WorkflowStarted"}, engine.DefaultLimits())
	if e, ok := err.(*ExecutionError); !ok || e.Code != ExecutionErrorPresentationSlotOccupied {
		t.Fatalf("expected ExecutionErrorPresentationSlotOccupied, got %v", err)
	}
}
