...
func TestNewSnapshot_MissingRootParameterRejected(t *testing.T) {
	p := compileRootWorkflowWithSlots(t)
	_, _, err := NewSnapshot(p, engine.InitializationInput{})
	if err == nil {
		t.Fatal("expected a missing-argument error")
	}
	execErr, ok := err.(*ExecutionError)
	if !ok || execErr.Code != ExecutionErrorInvalidInitialState {
		t.Fatalf("expected ExecutionErrorInvalidInitialState, got %v", err)
	}
}
...
func TestNewSnapshot_DoesNotExecuteAnyTransition(t *testing.T) {
	...
	// NewSnapshot must still be exactly the InitialState, untouched.
	def := program.Definition{
		Workflows: []program.WorkflowDeclaration{
			{
				Name:         "Main",
	...
	if signal.Name != "WorkflowStarted" {
		t.Fatalf("expected NewSnapshot to hand back the WorkflowStarted signal for a future Step, got %+v", signal)
	}
}
