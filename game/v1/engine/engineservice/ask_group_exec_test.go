...
// Compile, mirroring step_test.go's style): a "Main" workflow taking a
...
func answerAskGroup(p engine.Program, snap engine.Snapshot, respondent engine.UserID, answer bool) (engine.Commit, error) {
	return Step(p, snap, engine.Signal{Kind: engine.SignalKindAskGroupAnswered, Slot: "Ask", Respondent: respondent, Answer: engine.BoolValue{Value: answer}}, engine.DefaultLimits())
}
...
func TestExec_OpenAskGroupAllResponses_CompletesWhenEveryoneAnswers(t *testing.T) {
	...
	commit, err := Step(p, snap, engine.Signal{Name: "Open"}, engine.DefaultLimits())
	if err != nil {
	...
	commit, err = Step(p, snap, engine.Signal{Kind: engine.SignalKindAskGroupCompleted, Slot: "Ask"}, engine.DefaultLimits())
	if err != nil {
...
func TestExec_OpenAskGroupAllResponses_EmptyRecipientsCompletesImmediately(t *testing.T) {
	...
	_, err = Step(p, commit.Snapshot, engine.Signal{Kind: engine.SignalKindAskGroupCompleted, Slot: "Ask"}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error joining: %v", err)
	}
}
...
func TestExec_OpenAskGroupFirstResponse_CompletesOnFirstAnswer(t *testing.T) {
	...
	if err != ErrInputRejected {
		t.Fatalf("expected ErrInputRejected for an answer after first-response completion, got %v", err)
	}
}
...
func TestExec_OpenAskGroupFirstResponse_NoRecipientsIsExecutionError(t *testing.T) {
	...
	_, err := Step(p, snap, engine.Signal{Name: "Open"}, engine.DefaultLimits())
...
func TestExec_AcceptedResponseOrderingIsAcceptanceOrderNotRecipientOrder(t *testing.T) {
	...
	commit, _ := Step(p, snap, engine.Signal{Name: "Open"}, engine.DefaultLimits())
	snap = commit.Snapshot
...
func TestExec_DuplicateRecipientInOpenIsRejected(t *testing.T) {
	...
	_, err := Step(p, snap, engine.Signal{Name: "Open"}, engine.DefaultLimits())
	if e, ok := err.(*ExecutionError); !ok || e.Code != ExecutionErrorDuplicateRecipient {
		t.Fatalf("expected ExecutionErrorDuplicateRecipient, got %v", err)
	}
}
...
func TestExec_AnswerFromNonRecipientRejected(t *testing.T) {
	...
	if err != ErrInputRejected {
		t.Fatalf("expected ErrInputRejected for a non-recipient answer, got %v", err)
	}
}
...
func TestExec_DuplicateAnswerFromSameRespondentRejected(t *testing.T) {
	...
	if err != ErrInputRejected {
		t.Fatalf("expected ErrInputRejected for a duplicate answer, got %v", err)
	}
}
...
func TestExec_FinalizeCollectingGroupCompletesWithPartialResponses(t *testing.T) {
	...
	commit, err = Step(p, commit.Snapshot, engine.Signal{Name: "Finalize"}, engine.DefaultLimits())
	if err != nil {
...
func TestExec_FinalizeEmptySlotIsExecutionError(t *testing.T) {
	...
	_, err := Step(p, snap, engine.Signal{Name: "Finalize"}, engine.DefaultLimits())
...
func TestExec_CancelCollectingGroupDiscardsResponses(t *testing.T) {
	...
	commit, err = Step(p, commit.Snapshot, engine.Signal{Name: "CancelAsk"}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if commit.Snapshot.Root.AskGroupSlots[0].Pending != nil {
...
func TestExec_CancelEmptySlotIsNoOp(t *testing.T) {
	...
	commit, err := Step(p, snap, engine.Signal{Name: "CancelAsk"}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(commit.Outputs) != 0 {
...
func TestExec_CancelCompletedAwaitingJoinIsExecutionErrorAtomically(t *testing.T) {
	...
	_, err = Step(p, snap, engine.Signal{Name: "CancelAsk"}, engine.DefaultLimits())
	if e, ok := err.(*ExecutionError); !ok || e.Code != ExecutionErrorAskGroupNotJoined {
		t.Fatalf("expected ExecutionErrorAskGroupNotJoined, got %v", err)
	}
	if snap.Root.AskGroupSlots[0].Pending == nil {
...
func TestExec_JoinBindsResponsesRespondentsAndMissing(t *testing.T) {
	...
	commit, err = Step(p, commit.Snapshot, engine.Signal{Kind: engine.SignalKindAskGroupCompleted, Slot: "Ask"}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error joining: %v", err)
	}
...
func TestExec_JoinStaleOrDuplicateRejected(t *testing.T) {
	...
	// Not even opened yet.
	_, err := Step(p, snap, engine.Signal{Kind: engine.SignalKindAskGroupCompleted, Slot: "Ask"}, engine.DefaultLimits())
	if err != ErrInputRejected {
		t.Fatalf("expected ErrInputRejected joining an unopened slot, got %v", err)
	}
	...
	_, err = Step(p, commit.Snapshot, engine.Signal{Kind: engine.SignalKindAskGroupCompleted, Slot: "Ask"}, engine.DefaultLimits())
	if err != ErrInputRejected {
		t.Fatalf("expected ErrInputRejected joining a still-collecting slot, got %v", err)
	}
