...
// Compile, mirroring step_test.go's style) for a single workflow "QDemo"
...
func TestExec_CloseQuestionClearsOccupiedSlotAndOutput(t *testing.T) {
	...
	commit, err := Step(p, snap, engine.Signal{Name: "Close"}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestExec_QuestionAnsweredAcceptedClearsSlotAndDispatches(t *testing.T) {
	...
	commit, err := Step(p, snap, engine.Signal{Kind: engine.SignalKindQuestionAnswered, Slot: "Ask", Respondent: player, Answer: engine.BoolValue{Value: true}}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestExec_QuestionAnsweredWrongRespondentRejected(t *testing.T) {
	...
	_, err := Step(p, snap, engine.Signal{Kind: engine.SignalKindQuestionAnswered, Slot: "Ask", Respondent: other, Answer: engine.BoolValue{Value: true}}, engine.DefaultLimits())
	if err != ErrInputRejected {
		t.Fatalf("expected ErrInputRejected for an unauthorized respondent, got %v", err)
	}
}
...
func TestExec_QuestionAnsweredStaleOrDuplicateRejected(t *testing.T) {
	...
	_, err := Step(p, snap, engine.Signal{Kind: engine.SignalKindQuestionAnswered, Slot: "Ask", Respondent: player, Answer: engine.BoolValue{Value: true}}, engine.DefaultLimits())
	if err != ErrInputRejected {
		t.Fatalf("expected ErrInputRejected for a stale/duplicate answer, got %v", err)
	}
}
...
func TestExec_QuestionAnsweredDuplicateAfterAcceptanceRejected(t *testing.T) {
	...
	commit, err := Step(p, snap, answer, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error on first delivery: %v", err)
	}

	// Deliver the exact same answer again against the resulting snapshot.
	_, err = Step(p, commit.Snapshot, answer, engine.DefaultLimits())
	if err != ErrSignalRejected && err != ErrInputRejected {
...
func TestExec_QuestionAnsweredFailingValidationRejected(t *testing.T) {
	...
	_, err := Step(p, snap, engine.Signal{Kind: engine.SignalKindQuestionAnswered, Slot: "Ask", Respondent: player, Answer: engine.BoolValue{Value: false}}, engine.DefaultLimits())
	if err != ErrInputRejected {
		t.Fatalf("expected ErrInputRejected for an answer failing Validation, got %v", err)
	}
}
...
func TestExec_ScheduleTimerOccupiesSlotAndProducesOutput(t *testing.T) {
	...
	commit, err := Step(p, snap, engine.Signal{Name: "Schedule"}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestExec_ScheduleTimerOnOccupiedSlotFails(t *testing.T) {
	...
	_, err := Step(p, snap, engine.Signal{Name: "Schedule"}, engine.DefaultLimits())
	if e, ok := err.(*ExecutionError); !ok || e.Code != ExecutionErrorSlotOccupied {
		t.Fatalf("expected ExecutionErrorSlotOccupied, got %v", err)
	}
}
...
func TestExec_CancelTimerClearsOccupiedSlotAndOutput(t *testing.T) {
	...
	commit, err := Step(p, snap, engine.Signal{Name: "Cancel"}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	slot, _ := findInstanceTimerSlot(commit.Snapshot.Root, "Deadline")
...
func TestExec_TimerExpiredAcceptedClearsSlotAndDispatches(t *testing.T) {
	...
	commit, err := Step(p, snap, engine.Signal{Kind: engine.SignalKindTimerExpired, Slot: "Deadline"}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if commit.Snapshot.Root.Outcome == nil || commit.Snapshot.Root.Outcome.Kind != engine.WorkflowOutcomeCompleted {
...
func TestExec_TimerExpiredNotPendingRejected(t *testing.T) {
	...
	_, err := Step(p, snap, engine.Signal{Kind: engine.SignalKindTimerExpired, Slot: "Deadline"}, engine.DefaultLimits())
	if err != ErrInputRejected {
		t.Fatalf("expected ErrInputRejected for a not-pending timer, got %v", err)
	}
}
...
func TestExec_EmitEffectProducesOutput(t *testing.T) {
	...
	commit, err := Step(p, snap, engine.Signal{Name: "Emit"}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(commit.Outputs) != 1 {
...
func TestExec_InvariantViolationAfterOpenQuestionLeavesSlotsUntouched(t *testing.T) {
	...
	_, err := Step(p, snap, engine.Signal{Name: "Open"}, engine.DefaultLimits())
	if e, ok := err.(*ExecutionError); !ok || e.Code != ExecutionErrorInvariantViolation {
