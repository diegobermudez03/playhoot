...
func terminateTask(t *testing.T, p engine.Program, snap engine.Snapshot, key float64, intent string) engine.Snapshot {
	t.Helper()
	commit, err := Step(p, snap, engine.Signal{Kind: engine.SignalKindIntent, Path: taskPath(key), Intent: intent}, engine.DefaultLimits())
	if err != nil {
...
func TestExec_BeginAndSealTaskGroupCreatesRunningTasks(t *testing.T) {
	...
	commit, err := Step(p, snap, engine.Signal{Name: "BeginAndSeal"}, engine.DefaultLimits())
	if err != nil {
...
func TestExec_BeginIntoOccupiedSlotFailsAtomically(t *testing.T) {
	...
	snap := taskGroupSnapshot()
	commit, _ := Step(p, snap, engine.Signal{Name: "BeginAndSeal"}, engine.DefaultLimits())

	_, err := Step(p, commit.Snapshot, engine.Signal{Name: "BeginAndSeal"}, engine.DefaultLimits())
...
func TestExec_AllTerminalPolicy_CompletesWhenEveryTaskTerminates(t *testing.T) {
	p := taskGroupProgram(engine.TaskGroupAllTerminalPolicy{}, []float64{10, 20, 30}, engine.UnitType{}, engine.StayControl{})
	commit, _ := Step(p, taskGroupSnapshot(), engine.Signal{Name: "BeginAndSeal"}, engine.DefaultLimits())
...
func TestExec_FirstTerminalPolicy_CompletesOnFirstTerminationAndOthersUnfinished(t *testing.T) {
	...
	_, err := Step(p, snap, engine.Signal{Kind: engine.SignalKindIntent, Path: taskPath(1), Intent: "Succeed"}, engine.DefaultLimits())
...
func TestExec_FirstTerminalPolicy_NoTasksIsExecutionErrorAtSeal(t *testing.T) {
	...
	_, err := Step(p, taskGroupSnapshot(), engine.Signal{Name: "BeginAndSeal"}, engine.DefaultLimits())
	if e, ok := err.(*ExecutionError); !ok || e.Code != ExecutionErrorInvalidQuorum {
		t.Fatalf("expected ExecutionErrorInvalidQuorum, got %v", err)
	}
}
...
func TestExec_DuplicateTaskKeyIsRejected(t *testing.T) {
	...
	_, err := Step(p, taskGroupSnapshot(), engine.Signal{Name: "BeginAndSeal"}, engine.DefaultLimits())
	if e, ok := err.(*ExecutionError); !ok || e.Code != ExecutionErrorDuplicateTaskKey {
		t.Fatalf("expected ExecutionErrorDuplicateTaskKey, got %v", err)
	}
}
...
func TestExec_TaskGroupLeftBuildingIsRejectedAtomically(t *testing.T) {
	...
	snap := taskGroupSnapshot()
	_, err := Step(p, snap, engine.Signal{Name: "BeginAndSeal"}, engine.DefaultLimits())
	if e, ok := err.(*ExecutionError); !ok || e.Code != ExecutionErrorTaskGroupLeftBuilding {
		t.Fatalf("expected ExecutionErrorTaskGroupLeftBuilding, got %v", err)
	}
...
func TestExec_FinalizeRunningGroupCompletesWithPartialResultsAndUnfinished(t *testing.T) {
	...
	commit, err := Step(p, snap, engine.Signal{Name: "Finalize"}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	group := commit.Snapshot.Root.TaskGroupSlots[0].Group
...
func TestExec_TaskGroupFinalizeEmptyOrBuildingSlotIsExecutionError(t *testing.T) {
	p := taskGroupProgram(engine.TaskGroupAllTerminalPolicy{}, nil, engine.UnitType{}, engine.StayControl{})
	_, err := Step(p, taskGroupSnapshot(), engine.Signal{Name: "Finalize"}, engine.DefaultLimits())
	if e, ok := err.(*ExecutionError); !ok || e.Code != ExecutionErrorUnknown {
...
func TestExec_CancelBuildingOrRunningGroupDiscardsTasks(t *testing.T) {
	...
	commit, err := Step(p, snap, engine.Signal{Name: "CancelGroup"}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestExec_TaskGroupCancelEmptySlotIsNoOp(t *testing.T) {
	p := taskGroupProgram(engine.TaskGroupAllTerminalPolicy{}, nil, engine.UnitType{}, engine.StayControl{})
	commit, err := Step(p, taskGroupSnapshot(), engine.Signal{Name: "CancelGroup"}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if commit.Snapshot.Root.TaskGroupSlots[0].Group != nil {
...
func TestExec_TaskGroupCancelCompletedAwaitingJoinIsExecutionErrorAtomically(t *testing.T) {
	...
	_, err := Step(p, snap, engine.Signal{Name: "CancelGroup"}, engine.DefaultLimits())
	if e, ok := err.(*ExecutionError); !ok || e.Code != ExecutionErrorTaskGroupNotJoined {
		t.Fatalf("expected ExecutionErrorTaskGroupNotJoined, got %v", err)
	}
	if snap.Root.TaskGroupSlots[0].Group == nil {
...
func TestExec_TaskGroupJoinStaleOrDuplicateRejected(t *testing.T) {
	...
	_, err := Step(p, snap, engine.Signal{Kind: engine.SignalKindTaskGroupCompleted, Slot: "Workers"}, engine.DefaultLimits())
	if err != ErrInputRejected {
		t.Fatalf("expected ErrInputRejected joining an empty slot, got %v", err)
	}

	commit, _ := Step(p, snap, engine.Signal{Name: "BeginAndSeal"}, engine.DefaultLimits())
	_, err = Step(p, commit.Snapshot, engine.Signal{Kind: engine.SignalKindTaskGroupCompleted, Slot: "Workers"}, engine.DefaultLimits())
	if err != ErrInputRejected {
		t.Fatalf("expected ErrInputRejected joining a still-running group, got %v", err)
	}
...
func TestExec_RecursiveCleanupOnParentTerminationClearsTaskGroup(t *testing.T) {
	...
	commit, _ := Step(p, taskGroupSnapshot(), engine.Signal{Name: "BeginAndSeal"}, engine.DefaultLimits())
	commit, err := Step(p, commit.Snapshot, engine.Signal{Name: "Terminate"}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if commit.Snapshot.Root.TaskGroupSlots[0].Group != nil {
