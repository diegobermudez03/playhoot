...
func TestExec_WorkflowDepthLimitExceeded(t *testing.T) {
	...
	for depth := 0; depth < 2; depth++ {
		commit, err := Step(p, snap, engine.Signal{Name: "Spawn", Path: path}, limits)
		if err != nil {
			t.Fatalf("unexpected error spawning at depth %d: %v", depth, err)
		}
		started := commit.InternalSignals[0]
		commit, err = Step(p, commit.Snapshot, started, limits)
		if err != nil {
	...
	_, err := Step(p, snap, engine.Signal{Name: "Spawn", Path: path}, limits)
	if e, ok := err.(*ExecutionError); !ok || e.Code != ExecutionErrorWorkflowDepthExceeded {
		t.Fatalf("expected ExecutionErrorWorkflowDepthExceeded, got %v", err)
	}
	if e := err.(*ExecutionError); e.Code.String() != "workflow_depth_exceeded" {
...
func TestExec_ActiveSlotLimitExceeded(t *testing.T) {
	...
	_, err := Step(p, snap, engine.Signal{Name: "BeginAndSeal"}, limits)
	if e, ok := err.(*ExecutionError); !ok || e.Code != ExecutionErrorActiveSlotLimitExceeded {
		t.Fatalf("expected ExecutionErrorActiveSlotLimitExceeded, got %v", err)
	}
	...
	commit, err := Step(within, taskGroupSnapshot(), engine.Signal{Name: "BeginAndSeal"}, limits)
	if err != nil {
...
func TestExec_TraceRecordsTransitionGuardStateAndOutputs(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		commit, err = Step(p, commit.Snapshot, engine.Signal{Name: "Increment"}, engine.DefaultLimits())
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	}
	commit, err = Step(p, commit.Snapshot, engine.Signal{Name: "Finish"}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	tr = commit.Trace
...
func TestExec_ReplayIsDeterministic(t *testing.T) {
	...
	commit1, err1 := Step(p, snap, signal, engine.DefaultLimits())
	commit2, err2 := Step(p, snap, signal, engine.DefaultLimits())
	...
	commit3, err3 := Step(p, snap, signal, engine.DefaultLimits())
	if err3 != nil {
...
func TestExec_RetryAfterFailureIsSafeAndReproducible(t *testing.T) {
	...
	_, err1 := Step(p, snap, signal, engine.DefaultLimits())
	_, err2 := Step(p, snap, signal, engine.DefaultLimits())
	if err1 != ErrSignalRejected || err2 != ErrSignalRejected {
...
func TestExec_SignalToNonexistentPathIsSignalRejectedNotInputRejected(t *testing.T) {
	...
	_, err := Step(p, snap, engine.Signal{Name: "WorkflowStarted", Path: []engine.PathStep{{Slot: "W"}}}, engine.DefaultLimits())
	if !errors.Is(err, ErrSignalRejected) {
		t.Fatalf("expected ErrSignalRejected for an unaddressable path, got %v", err)
	}
	if errors.Is(err, ErrInputRejected) {
		t.Fatal("a bad path must not be reported as ErrInputRejected — those are structurally different outcomes")
	}
}

func TestExec_ExecutionErrorCodeStringIsExhaustiveAndStable(t *testing.T) {
	codes := []ExecutionErrorCode{
		ExecutionErrorUnknown, ExecutionErrorNotImplemented, ExecutionErrorUndefinedReference,
		ExecutionErrorDivisionByZero, ExecutionErrorIndexOutOfRange, ExecutionErrorKeyNotFound,
		ExecutionErrorNoMatchingCase, ExecutionErrorInvalidInitialState, ExecutionErrorInvariantViolation,
		ExecutionErrorSnapshotProgramMismatch, ExecutionErrorSignalRejected, ExecutionErrorBudgetExceeded,
		ExecutionErrorLoopLimitExceeded, ExecutionErrorInvalidRandomRange, ExecutionErrorEmptyRandomCollection,
		ExecutionErrorSlotOccupied, ExecutionErrorInvalidTimerDelay, ExecutionErrorInputRejected,
		ExecutionErrorChildOutcomeNotJoined, ExecutionErrorDuplicateRecipient, ExecutionErrorInvalidQuorum,
		ExecutionErrorAskGroupNotJoined, ExecutionErrorDuplicateTaskKey, ExecutionErrorTaskGroupNotJoined,
		ExecutionErrorTaskGroupLeftBuilding, ExecutionErrorPresentationSlotOccupied,
		ExecutionErrorWorkflowDepthExceeded, ExecutionErrorActiveSlotLimitExceeded,
	}
	seen := make(map[string]ExecutionErrorCode, len(codes))
	for _, c := range codes {
		name := c.String()
		if name == "" {
			t.Fatalf("code %d has an empty String()", c)
		}
		if c != ExecutionErrorUnknown && name == "unknown" {
...
func TestExec_InvariantFailureAfterTransitionIsAtomicAndReproducible(t *testing.T) {
	...
	_, err1 := Step(p, snap, engine.Signal{Name: "Increment"}, engine.DefaultLimits())
	_, err2 := Step(p, snap, engine.Signal{Name: "Increment"}, engine.DefaultLimits())
	e1, ok1 := err1.(*ExecutionError)
	e2, ok2 := err2.(*ExecutionError)
	if !ok1 || !ok2 || e1.Code != ExecutionErrorInvariantViolation || e2.Code != ExecutionErrorInvariantViolation {
		t.Fatalf("expected two identical ExecutionErrorInvariantViolation, got %v, %v", err1, err2)
	}
	count, _ := snap.GlobalState.FieldByName("count")
