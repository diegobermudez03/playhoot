...
func TestStep_DrawRandomIntegerIsDeterministicAndInRange(t *testing.T) {
	...
	commit1, err := runtime.Step(p, snap, engine.Signal{Name: "Roll"}, engine.DefaultLimits())
	if err != nil {
	...
	commit2, err := runtime.Step(p, snap, engine.Signal{Name: "Roll"}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestStep_DrawRandomEmptyCollectionErrors(t *testing.T) {
	...
	_, err := runtime.Step(p, snap, engine.Signal{Name: "Go"}, engine.DefaultLimits())
	execErr, ok := err.(*runtime.ExecutionError)
	if !ok || execErr.Code != runtime.ExecutionErrorEmptyRandomCollection {
		t.Fatalf("expected runtime.ExecutionErrorEmptyRandomCollection, got %v", err)
	}
}
...
func TestStep_ExecutionBudgetExceeded(t *testing.T) {
	...
	// of 100 must trip.
	_, err := runtime.Step(p, snap, engine.Signal{Name: "Go"}, engine.Limits{MaxOperations: 100, MaxLoopIterations: 10_000})
	execErr, ok := err.(*runtime.ExecutionError)
	if !ok || execErr.Code != runtime.ExecutionErrorBudgetExceeded {
		t.Fatalf("expected runtime.ExecutionErrorBudgetExceeded, got %v", err)
	}
}
...
func TestStep_LoopLimitExceeded(t *testing.T) {
	...
	_, err := runtime.Step(p, snap, engine.Signal{Name: "Go"}, engine.Limits{MaxOperations: 10_000, MaxLoopIterations: 10})
	execErr, ok := err.(*runtime.ExecutionError)
	if !ok || execErr.Code != runtime.ExecutionErrorLoopLimitExceeded {
		t.Fatalf("expected runtime.ExecutionErrorLoopLimitExceeded, got %v", err)
	}
}
