...
func spawnAndStart(t *testing.T, p engine.Program, snap engine.Snapshot) engine.Snapshot {
	t.Helper()
	commit, err := runtime.Step(p, snap, engine.Signal{Name: "Spawn"}, engine.DefaultLimits())
	if err != nil {
	...
	commit, err = runtime.Step(p, commit.Snapshot, started, engine.DefaultLimits())
	if err != nil {
...
func TestExec_SpawnIntoOccupiedSlotFailsAtomically(t *testing.T) {
	...
	_, err := runtime.Step(p, snap, engine.Signal{Name: "Spawn"}, engine.DefaultLimits())
...
func TestExec_ChildCompletesAndParentJoins(t *testing.T) {
	...
	commit, err := runtime.Step(p, snap, engine.Signal{Kind: engine.SignalKindIntent, Path: []engine.PathStep{{Slot: "W"}}, Intent: "Succeed"}, engine.DefaultLimits())
	if err != nil {
	...
	_, err = runtime.Step(p, commit.Snapshot, engine.Signal{Kind: engine.SignalKindChildCompleted, Slot: "W"}, engine.DefaultLimits())
...
func TestExec_ChildFailsAndParentJoins(t *testing.T) {
	...
	commit, err := runtime.Step(p, snap, engine.Signal{Kind: engine.SignalKindIntent, Path: []engine.PathStep{{Slot: "W"}}, Intent: "Fail"}, engine.DefaultLimits())
	if err != nil {
	...
	commit, err = runtime.Step(p, snap, engine.Signal{Kind: engine.SignalKindChildFailed, Slot: "W"}, engine.DefaultLimits())
	if err != nil {
...
func TestExec_ChildSelfCancelsAndParentJoins(t *testing.T) {
	...
	commit, err := runtime.Step(p, snap, engine.Signal{Kind: engine.SignalKindIntent, Path: []engine.PathStep{{Slot: "W"}}, Intent: "SelfCancel"}, engine.DefaultLimits())
	if err != nil {
	...
	commit, err = runtime.Step(p, snap, engine.Signal{Kind: engine.SignalKindChildCancelled, Slot: "W"}, engine.DefaultLimits())
	if err != nil {
...
func TestExec_ChildOutcomeWrongKindRejected(t *testing.T) {
	...
	_, err = runtime.Step(p, commit.Snapshot, engine.Signal{Kind: engine.SignalKindChildFailed, Slot: "W"}, engine.DefaultLimits())
	if err != runtime.ErrInputRejected {
...
func TestExec_ParentDrivenCancelDiscardsRunningChild(t *testing.T) {
	...
	commit, err := runtime.Step(p, snap, engine.Signal{Name: "CancelChild"}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if commit.Snapshot.Root.ChildSlots[0].Child != nil {
...
func TestExec_ParentDrivenCancelOnTerminalAwaitingJoinFailsAtomically(t *testing.T) {
	...
	_, err = runtime.Step(p, snap, engine.Signal{Name: "CancelChild"}, engine.DefaultLimits())
	if e, ok := err.(*runtime.ExecutionError); !ok || e.Code != runtime.ExecutionErrorChildOutcomeNotJoined {
		t.Fatalf("expected runtime.ExecutionErrorChildOutcomeNotJoined, got %v", err)
	}
...
func TestExec_RecursiveCleanupOnParentTermination_RunningChild(t *testing.T) {
	...
	commit, err := runtime.Step(p, snap, engine.Signal{Name: "Terminate"}, engine.DefaultLimits())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestExec_SignalToNonexistentChildPathIsRejected(t *testing.T) {
	...
	_, err := runtime.Step(p, snap, engine.Signal{Kind: engine.SignalKindIntent, Path: []engine.PathStep{{Slot: "W"}}, Intent: "Succeed"}, engine.DefaultLimits())
	if err != runtime.ErrSignalRejected {
		t.Fatalf("expected runtime.ErrSignalRejected for a signal addressed to a nonexistent child, got %v", err)
	}
}
