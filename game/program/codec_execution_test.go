...
func TestAssignmentTarget_RoundTrip_DeeplyNested(t *testing.T) {
	original := FieldTarget{
		Target: IndexTarget{
			Target: FieldTarget{
				Target: NameTarget{Name: "global"},
				Field:  "participants",
			},
...
func TestAssignmentTarget_AllVariants(t *testing.T) {
	cases := []AssignmentTarget{
		NameTarget{Name: "global"},
		FieldTarget{Target: NameTarget{Name: "global"}, Field: "participants"},
		IndexTarget{Target: NameTarget{Name: "turnOrder"}, Index: NumberLiteralExpression{Value: "0"}},
	}
...
func TestBlock_RoundTrip_NilVsEmptyOperations(t *testing.T) {
	...
	emptyBlock := Block{Operations: []Operation{}}
...
func TestBlock_RoundTrip_OrderedOperations(t *testing.T) {
	original := Block{
		Operations: []Operation{
			SetOperation{Target: NameTarget{Name: "a"}, Value: NumberLiteralExpression{Value: "1"}},
			SetOperation{Target: NameTarget{Name: "b"}, Value: NumberLiteralExpression{Value: "2"}},
			SetOperation{Target: NameTarget{Name: "c"}, Value: NumberLiteralExpression{Value: "3"}},
		},
	}
	...
	for i, name := range []string{"a", "b", "c"} {
		op := decoded.Operations[i].(SetOperation)
		if op.Target.(NameTarget).Name != name {
			t.Fatalf("operation %d: expected target %q, got %q", i, name, op.Target.(NameTarget).Name)
		}
	}
}
...
func TestBlock_RoundTrip_DeeplyNested(t *testing.T) {
	original := Block{
		Operations: []Operation{
			IfOperation{
				Condition: ReferenceExpression{Name: "condition"},
				Then: Block{
					Operations: []Operation{
						ForEachOperation{
							...
							Body: Block{
								Operations: []Operation{
									MatchOperation{
										Value: ReferenceExpression{Name: "participant"},
										Cases: []MatchOperationCase{
											{
												Pattern: WildcardMatchPattern{},
												Body: Block{
													Operations: []Operation{
														SetOperation{Target: NameTarget{Name: "found"}, Value: BoolLiteralExpression{Value: true}},
													},
												},
											},
										},
...
func TestBlock_DecodeNull_IsStructuralError(t *testing.T) {
	...
	if err == nil {
		t.Fatal("expected an error decoding null as Block")
	}
...
func allOperationVariants() []Operation {
	return []Operation{
		LetOperation{
			Name:  "participant",
			Type:  NamedTypeReference{Name: "Participant"},
			Value: ReferenceExpression{Name: "currentUser"},
		},
		SetOperation{
			Target: FieldTarget{Target: NameTarget{Name: "global"}, Field: "turnNumber"},
			Value:  NumberLiteralExpression{Value: "1"},
		},
		ListAppendOperation{
			Target: FieldTarget{Target: NameTarget{Name: "global"}, Field: "turnOrder"},
			Value:  ReferenceExpression{Name: "currentUser"},
		},
		ListInsertOperation{
			Target: FieldTarget{Target: NameTarget{Name: "global"}, Field: "turnOrder"},
			Index:  NumberLiteralExpression{Value: "0"},
			Value:  ReferenceExpression{Name: "currentUser"},
		},
		ListRemoveAtOperation{
			Target: NameTarget{Name: "turnOrder"},
			Index:  NumberLiteralExpression{Value: "2"},
		},
		MapPutOperation{
			Target: FieldTarget{Target: NameTarget{Name: "global"}, Field: "scores"},
			Key:    ReferenceExpression{Name: "currentUser"},
		...
		IfOperation{
			Condition: ReferenceExpression{Name: "condition"},
			Then:      Block{Operations: []Operation{}},
			Else:      Block{Operations: []Operation{}},
		},
		ForEachOperation{
			Collection: ReferenceExpression{Name: "participants"},
			ItemName:   "participant",
			IndexName:  "index",
			Body:       Block{Operations: []Operation{}},
		},
		MatchOperation{
			Value: ReferenceExpression{Name: "turnResult"},
			Cases: []MatchOperationCase{
				{
					Pattern: UnionVariantMatchPattern{
						TypeName:    "TurnResult",
						VariantName: "Won",
						Bindings:    []MatchFieldBinding{{Field: "participant", Name: "winner"}},
					},
					Body: Block{
						Operations: []Operation{
							SetOperation{
								Target: FieldTarget{Target: NameTarget{Name: "global"}, Field: "winner"},
								Value:  ReferenceExpression{Name: "winner"},
							},
						},
					},
				},
		...
		OpenQuestionOperation{
			Slot:      "moveRequest",
			Recipient: ReferenceExpression{Name: "participant"},
			Arguments: []CallArgument{
				{Name: "plans", Value: FieldExpression{Target: ReferenceExpression{Name: "local"}, Field: "availablePlans"}},
			},
		},
		CloseQuestionOperation{Slot: "moveRequest"},
		EmitEffectOperation{
			Effect:     "DiceRolled",
			Recipients: ReferenceExpression{Name: "roomUsers"},
			Arguments: []CallArgument{
				{Name: "first", Value: ReferenceExpression{Name: "firstDie"}},
				{Name: "second", Value: ReferenceExpression{Name: "secondDie"}},
			},
		},
		ScheduleTimerOperation{
			Slot:              "moveDeadline",
			DelayMilliseconds: NumberLiteralExpression{Value: "30000"},
		},
		CancelTimerOperation{Slot: "moveDeadline"},
		SpawnChildWorkflowOperation{
			Slot: "activeTurn",
			Arguments: []CallArgument{
				{Name: "participant", Value: ReferenceExpression{Name: "participantId"}},
			},
		},
		CancelChildWorkflowOperation{
			Slot:   "activeTurn",
			Reason: StringLiteralExpression{Value: "participant disconnected"},
		},
		OpenAskGroupOperation{
			Slot:       "cardVotes",
			Recipients: FieldExpression{Target: ReferenceExpression{Name: "team"}, Field: "users"},
			Arguments: []CallArgument{
				{Name: "options", Value: FieldExpression{Target: ReferenceExpression{Name: "local"}, Field: "availableCards"}},
			},
			Completion: AskGroupQuorumPolicy{Count: ReferenceExpression{Name: "requiredVotes"}},
		},
		FinalizeAskGroupOperation{Slot: "cardVotes"},
		CancelAskGroupOperation{Slot: "cardVotes"},
		BeginTaskGroupOperation{
			Slot:       "teamSelections",
			Completion: TaskGroupAllTerminalPolicy{},
		},
		...
		FinalizeTaskGroupOperation{Slot: "teamSelections"},
		CancelTaskGroupOperation{
			Slot:   "teamSelections",
			Reason: StringLiteralExpression{Value: "round cancelled"},
		},
		DrawRandomOperation{
			Name: "firstDie",
...
func TestOperation_AllVariants_RoundTrip(t *testing.T) {
	...
	if len(variants) != len(wantedOperationKinds) {
		t.Fatalf("expected %d operation variants covered, got %d — update allOperationVariants to cover every Operation kind", len(wantedOperationKinds), len(variants))
	}
...
func TestWorkflowControl_AllVariants_RoundTrip(t *testing.T) {
	variants := []WorkflowControl{
		GotoControl{State: "WaitingForMove"},
		StayControl{},
		CompleteControl{Result: ReferenceExpression{Name: "selectedMove"}},
		FailControl{Error: StringLiteralExpression{Value: "unable to resolve turn"}},
		CancelControl{Reason: StringLiteralExpression{Value: "participant disconnected"}},
		ConditionalControl{
			Condition: ReferenceExpression{Name: "hasWinner"},
			Then:      CompleteControl{Result: ReferenceExpression{Name: "winner"}},
			Else:      GotoControl{State: "NextRound"},
		},
		MatchControl{
			Value: ReferenceExpression{Name: "turnResult"},
			Cases: []MatchControlCase{
				{
					...
					Control: CompleteControl{Result: ReferenceExpression{Name: "winner"}},
				},
				{
					Pattern: WildcardMatchPattern{},
					Control: GotoControl{State: "NextTurn"},
				},
			},
		},
	}
...
func TestSignalSource_AllVariants_RoundTrip(t *testing.T) {
	variants := []SignalSource{
		NamedSignalSource{Name: "WorkflowStarted"},
		UserIntentSignalSource{Intent: "PlayCard"},
		QuestionAnsweredSignalSource{Slot: "moveRequest"},
		TimerExpiredSignalSource{Slot: "moveDeadline"},
		ChildCompletedSignalSource{Slot: "activeTurn"},
		ChildFailedSignalSource{Slot: "activeTurn"},
		ChildCancelledSignalSource{Slot: "activeTurn"},
		AskGroupCompletedSignalSource{Slot: "cardVotes"},
		TaskGroupCompletedSignalSource{Slot: "teamSelections"},
	}
...
func TestSignalPattern_NilVsEmptyBindings(t *testing.T) {
	nilBindings := SignalPattern{Source: NamedSignalSource{Name: "WorkflowStarted"}, Bindings: nil}
	...
	emptyBindings := SignalPattern{Source: NamedSignalSource{Name: "WorkflowStarted"}, Bindings: []SignalBinding{}}
...
func TestSignalPattern_DecodeNull_IsStructuralError(t *testing.T) {
	...
	if err == nil {
		t.Fatal("expected an error decoding null as SignalPattern")
	}
	var decodeErr *DecodeError
	if !errors.As(err, &decodeErr) {
		t.Fatalf("expected *DecodeError, got %T: %v", err, err)
...
func TestAskGroupCompletionPolicy_AllVariants_RoundTrip(t *testing.T) {
	variants := []AskGroupCompletionPolicy{
		AskGroupAllResponsesPolicy{},
		AskGroupFirstResponsePolicy{},
		AskGroupQuorumPolicy{Count: ReferenceExpression{Name: "requiredVotes"}},
	}
	for _, original := range variants {
...
func TestTaskGroupCompletionPolicy_AllVariants_RoundTrip(t *testing.T) {
	variants := []TaskGroupCompletionPolicy{
		TaskGroupAllTerminalPolicy{},
		TaskGroupFirstTerminalPolicy{},
		TaskGroupQuorumTerminalPolicy{Count: ReferenceExpression{Name: "requiredTasks"}},
	}
	for _, original := range variants {
...
func TestExactJSON_SetOperation(t *testing.T) {
	raw, err := encodeOperation("$", SetOperation{
		Target: FieldTarget{Target: NameTarget{Name: "global"}, Field: "turnNumber"},
		Value:  NumberLiteralExpression{Value: "1"},
	})
	if err != nil {
...
func TestExactJSON_IfOperation(t *testing.T) {
	raw, err := encodeOperation("$", IfOperation{
		Condition: BoolLiteralExpression{Value: true},
		Then:      Block{Operations: []Operation{}},
		Else:      Block{},
	})
	if err != nil {
	...
	if elseBlock["operations"] != nil {
		t.Fatalf("expected else.operations to be null (zero-value Block), got %#v", elseBlock["operations"])
	}
}

func TestExactJSON_OpenAskGroupOperation(t *testing.T) {
	raw, err := encodeOperation("$", OpenAskGroupOperation{
		Slot:       "cardVotes",
		Recipients: FieldExpression{Target: ReferenceExpression{Name: "team"}, Field: "users"},
		Arguments: []CallArgument{
			{Name: "options", Value: FieldExpression{Target: ReferenceExpression{Name: "local"}, Field: "availableCards"}},
...
func TestExactJSON_BeginTaskGroupOperation(t *testing.T) {
	raw, err := encodeOperation("$", BeginTaskGroupOperation{
		Slot:       "teamSelections",
		Completion: TaskGroupAllTerminalPolicy{},
	})
	if err != nil {
...
func TestExactJSON_DrawRandomOperation(t *testing.T) {
	raw, err := encodeOperation("$", DrawRandomOperation{
		Name: "firstDie",
		Generator: RandomIntegerGenerator{
			Minimum: NumberLiteralExpression{Value: "1"},
			Maximum: NumberLiteralExpression{Value: "6"},
...
func TestExactJSON_ConditionalControl(t *testing.T) {
	raw, err := encodeWorkflowControl("$", ConditionalControl{
		Condition: ReferenceExpression{Name: "hasWinner"},
		Then:      CompleteControl{Result: ReferenceExpression{Name: "winner"}},
		Else:      GotoControl{State: "NextRound"},
	})
...
func TestExactJSON_MatchControl(t *testing.T) {
	raw, err := encodeWorkflowControl("$", MatchControl{
		Value: ReferenceExpression{Name: "turnResult"},
		Cases: []MatchControlCase{
			{Pattern: WildcardMatchPattern{}, Control: StayControl{}},
		},
	})
	if err != nil {
		t.Fatalf("encode: %v", err)
...
func TestExactJSON_UserIntentSignalSource(t *testing.T) {
	raw, err := encodeSignalSource("$", UserIntentSignalSource{Intent: "PlayCard"})
	if err != nil {
		t.Fatalf("encode: %v", err)
	}
	var obj map[string]any
...
func TestExactJSON_AskGroupQuorumPolicy(t *testing.T) {
	raw, err := encodeAskGroupCompletionPolicy("$", AskGroupQuorumPolicy{Count: ReferenceExpression{Name: "requiredVotes"}})
	if err != nil {
		t.Fatalf("encode: %v", err)
	}
	var obj map[string]any
...
func TestExactJSON_TaskGroupQuorumTerminalPolicy(t *testing.T) {
	raw, err := encodeTaskGroupCompletionPolicy("$", TaskGroupQuorumTerminalPolicy{Count: ReferenceExpression{Name: "requiredTasks"}})
	if err != nil {
		t.Fatalf("encode: %v", err)
	}
	var obj map[string]any
...
func TestExactJSON_DeeplyNestedAssignmentTarget(t *testing.T) {
	raw, err := encodeAssignmentTarget("$", FieldTarget{
		Target: IndexTarget{
			Target: FieldTarget{Target: NameTarget{Name: "global"}, Field: "participants"},
			Index:  ReferenceExpression{Name: "currentUser"},
		},
		Field: "score",
	})
...
func TestPointerAndValueEquivalence_Operation(t *testing.T) {
	value := SetOperation{Target: NameTarget{Name: "x"}, Value: NumberLiteralExpression{Value: "1"}}
...
func TestPointerAndValueEquivalence_WorkflowControl(t *testing.T) {
	value := GotoControl{State: "NextState"}
...
func TestPointerAndValueEquivalence_SignalSource(t *testing.T) {
	value := NamedSignalSource{Name: "WorkflowStarted"}
...
func TestPointerAndValueEquivalence_AskGroupPolicy(t *testing.T) {
	value := AskGroupAllResponsesPolicy{}
...
func TestPointerAndValueEquivalence_TaskGroupPolicy(t *testing.T) {
	value := TaskGroupAllTerminalPolicy{}
...
func TestTypedNilPointers_EncodeToNull(t *testing.T) {
	...
	var taskPolicy *TaskGroupAllTerminalPolicy
...
func TestDecode_UnknownDiscriminator_ExecutionFamilies(t *testing.T) {
	cases := []struct {
		...
		{"AssignmentTarget", func(p string, d json.RawMessage) error { _, err := decodeAssignmentTarget(p, d); return err }},
		{"Operation", func(p string, d json.RawMessage) error { _, err := decodeOperation(p, d); return err }},
		{"WorkflowControl", func(p string, d json.RawMessage) error { _, err := decodeWorkflowControl(p, d); return err }},
		{"SignalSource", func(p string, d json.RawMessage) error { _, err := decodeSignalSource(p, d); return err }},
		{"AskGroupCompletionPolicy", func(p string, d json.RawMessage) error { _, err := decodeAskGroupCompletionPolicy(p, d); return err }},
		{"TaskGroupCompletionPolicy", func(p string, d json.RawMessage) error { _, err := decodeTaskGroupCompletionPolicy(p, d); return err }},
	}
...
func TestSemanticInvalidity_EmptyGotoState(t *testing.T) {
	...
	if decoded.(GotoControl).State != "" {
		t.Fatalf("expected empty state preserved, got %q", decoded.(GotoControl).State)
	}
}
...
func TestSemanticInvalidity_UnknownSlot(t *testing.T) {
	...
	if decoded.(CloseQuestionOperation).Slot != "unknownSlot" {
		t.Fatalf("expected unknown slot preserved, got %q", decoded.(CloseQuestionOperation).Slot)
	}
}
...
func TestSemanticInvalidity_DuplicateCallArguments(t *testing.T) {
	...
	op := decoded.(OpenQuestionOperation)
...
func TestSemanticInvalidity_NilTimerDelay(t *testing.T) {
	...
	if decoded.(ScheduleTimerOperation).DelayMilliseconds != nil {
		t.Fatalf("expected nil delay preserved, got %#v", decoded.(ScheduleTimerOperation).DelayMilliseconds)
	}
}
...
func TestSemanticInvalidity_NegativeQuorumCount(t *testing.T) {
	...
	count := decoded.(AskGroupQuorumPolicy).Count.(NumberLiteralExpression)
	if count.Value != "-1" {
...
func TestSemanticInvalidity_NilTaskGroupSpawnKey(t *testing.T) {
	...
	if decoded.(SpawnTaskGroupChildOperation).Key != nil {
		t.Fatalf("expected nil key preserved, got %#v", decoded.(SpawnTaskGroupChildOperation).Key)
	}
}
...
func TestSemanticInvalidity_FailControlWithNumericExpression(t *testing.T) {
	...
	errExpr := decoded.(FailControl).Error.(NumberLiteralExpression)
...
func TestSemanticInvalidity_MatchOperationNoCases(t *testing.T) {
	...
	if len(decoded.(MatchOperation).Cases) != 0 {
		t.Fatalf("expected 0 cases preserved, got %d", len(decoded.(MatchOperation).Cases))
	}
}
...
func TestSemanticInvalidity_EmptyAssignmentTargetNames(t *testing.T) {
	...
	target := decoded.(FieldTarget)
	if target.Field != "" || target.Target.(NameTarget).Name != "" {
...
func TestNilVsEmptySlices_MatchOperationCases(t *testing.T) {
	nilCase := MatchOperation{Value: ReferenceExpression{Name: "x"}, Cases: nil}
	...
	if decoded.(MatchOperation).Cases != nil {
		t.Fatalf("expected nil cases preserved, got %#v", decoded.(MatchOperation).Cases)
	}

	emptyCase := MatchOperation{Value: ReferenceExpression{Name: "x"}, Cases: []MatchOperationCase{}}
	...
	cases := decoded.(MatchOperation).Cases
...
func TestDuplicateOrdering_TaskSpawnArguments(t *testing.T) {
	original := SpawnTaskGroupChildOperation{
		Slot: "teamSelections",
		Key:  ReferenceExpression{Name: "team"},
		Arguments: []CallArgument{
	...
	op := decoded.(SpawnTaskGroupChildOperation)
	if len(op.Arguments) != 2 || op.Arguments[0].Value.(NumberLiteralExpression).Value != "1" || op.Arguments[1].Value.(NumberLiteralExpression).Value != "2" {
		t.Fatalf("argument order not preserved: %#v", op.Arguments)
	}
}

func TestDuplicateOrdering_MatchCases(t *testing.T) {
	original := MatchOperation{
		Value: ReferenceExpression{Name: "x"},
		Cases: []MatchOperationCase{
			{Pattern: EnumValueMatchPattern{TypeName: "T", ValueName: "A"}, Body: Block{}},
			{Pattern: EnumValueMatchPattern{TypeName: "T", ValueName: "A"}, Body: Block{}},
		},
	}
	raw, err := encodeOperation("$", original)
	...
	if len(decoded.(MatchOperation).Cases) != 2 {
		t.Fatalf("expected 2 duplicate cases preserved, got %d", len(decoded.(MatchOperation).Cases))
	}
}
...
func TestIntegratedStructure_TransitionFragment(t *testing.T) {
	signal := SignalPattern{Source: UserIntentSignalSource{Intent: "RollDice"}}

	block := Block{
		Operations: []Operation{
			DrawRandomOperation{
				Name:      "firstDie",
				Generator: RandomIntegerGenerator{Minimum: NumberLiteralExpression{Value: "1"}, Maximum: NumberLiteralExpression{Value: "6"}},
			},
			DrawRandomOperation{
				Name:      "secondDie",
				Generator: RandomIntegerGenerator{Minimum: NumberLiteralExpression{Value: "1"}, Maximum: NumberLiteralExpression{Value: "6"}},
			...
			SetOperation{
				Target: FieldTarget{Target: NameTarget{Name: "global"}, Field: "lastRoll"},
				Value:  ReferenceExpression{Name: "firstDie"},
			},
			EmitEffectOperation{
				Effect:     "DiceRolled",
				Recipients: ReferenceExpression{Name: "roomUsers"},
	...
	control := GotoControl{State: "WaitingForMove"}
	...
	firstDraw := decodedBlock.Operations[0].(DrawRandomOperation)
	secondDraw := decodedBlock.Operations[1].(DrawRandomOperation)
	effect := decodedBlock.Operations[3].(EmitEffectOperation)
	if firstDraw.Name != effect.Arguments[0].Value.(ReferenceExpression).Name {
		t.Fatalf("expected effect first argument to reference %q, got %q", firstDraw.Name, effect.Arguments[0].Value.(ReferenceExpression).Name)
	}
	if secondDraw.Name != effect.Arguments[1].Value.(ReferenceExpression).Name {
		t.Fatalf("expected effect second argument to reference %q, got %q", secondDraw.Name, effect.Arguments[1].Value.(ReferenceExpression).Name)
	}

	// Cross-reference: the goto control's target state name is preserved
	// verbatim as authored.
	if decodedControl.(GotoControl).State != "WaitingForMove" {
		t.Fatalf("expected goto state %q, got %q", "WaitingForMove", decodedControl.(GotoControl).State)
	}
}
