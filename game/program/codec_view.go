...
func encodeViewDeclaration(path string, value ViewDeclaration) (json.RawMessage, error) {
...
// decodeViewDeclaration decodes data as a ViewDeclaration. JSON null is
...
func decodeViewDeclaration(path string, data json.RawMessage) (ViewDeclaration, error) {
	var wire wireViewDeclaration
	if err := decodeOrdinaryObject(path, data, &wire); err != nil {
		return ViewDeclaration{}, err
	}
	...
	return ViewDeclaration{Name: wire.Name, ModelType: modelType, LocalState: localState, Root: root}, nil
}

func encodeViewDeclarations(path string, items []ViewDeclaration) ([]json.RawMessage, error) {
	if items == nil {
		return nil, nil
	}
...
func decodeViewDeclarations(path string, items []json.RawMessage) ([]ViewDeclaration, error) {
	if items == nil {
		return nil, nil
	}
	result := make([]ViewDeclaration, len(items))
