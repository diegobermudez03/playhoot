...
func encodeResourceDeclaration(path string, value ResourceDeclaration) (json.RawMessage, error) {
...
// decodeResourceDeclaration decodes data as a ResourceDeclaration. JSON
// null is not a valid encoding and produces a path-aware structural error.
func decodeResourceDeclaration(path string, data json.RawMessage) (ResourceDeclaration, error) {
	...
	return ResourceDeclaration{Name: wire.Name, Type: typeRef, Value: val}, nil
}

func encodeResourceDeclarations(path string, items []ResourceDeclaration) ([]json.RawMessage, error) {
	if items == nil {
		return nil, nil
	}
...
func decodeResourceDeclarations(path string, items []json.RawMessage) ([]ResourceDeclaration, error) {
	if items == nil {
		return nil, nil
	}
	result := make([]ResourceDeclaration, len(items))
...
// --- StateFieldDeclaration / StateDeclaration ---
...
func encodeStateFieldDeclarations(path string, fields []StateFieldDeclaration) ([]wireStateFieldDeclaration, error) {
	if fields == nil {
		return nil, nil
	}
...
func decodeStateFieldDeclarations(path string, fields []wireStateFieldDeclaration) ([]StateFieldDeclaration, error) {
	if fields == nil {
		return nil, nil
	}
	result := make([]StateFieldDeclaration, len(fields))
	for i, field := range fields {
		itemPath := pathIndex(path, i)
		...
		result[i] = StateFieldDeclaration{Name: field.Name, Type: typeRef, Initializer: initializer}
	}
	return result, nil
}
...
// struct, as its JSON wire representation.
func encodeStateDeclaration(path string, value StateDeclaration) (json.RawMessage, error) {
...
// decodeStateDeclaration decodes data as a StateDeclaration. JSON null is
// not a valid encoding and produces a path-aware structural error.
func decodeStateDeclaration(path string, data json.RawMessage) (StateDeclaration, error) {
	var wire wireStateDeclaration
	if err := decodeOrdinaryObject(path, data, &wire); err != nil {
		return StateDeclaration{}, err
	}
	fields, err := decodeStateFieldDeclarations(pathField(path, "fields"), wire.Fields)
	if err != nil {
		return StateDeclaration{}, err
	}
	return StateDeclaration{Fields: fields}, nil
}
