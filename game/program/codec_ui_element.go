...
func encodeUIElementSlice(path string, items []UIElement) ([]json.RawMessage, error) {
	if items == nil {
		return nil, nil
	}
...
func decodeUIElementSlice(path string, items []json.RawMessage) ([]UIElement, error) {
	if items == nil {
		return nil, nil
	}
	result := make([]UIElement, len(items))
...
func encodeUIElement(path string, value UIElement) (json.RawMessage, error) {
	return encodeUnion(value, func(resolved any) (json.RawMessage, error) {
		...
		default:
			return nil, fmt.Errorf("%s: unsupported UIElement implementation %T", path, value)
		}
	})
}

// decodeUIElement decodes data as a UIElement, or returns a nil interface
// for JSON null or a missing value.
func decodeUIElement(path string, data json.RawMessage) (UIElement, error) {
	return decodeUnion(path, data, func(path, kind string, raw json.RawMessage) (UIElement, error) {
		switch kind {
		case "empty":
		...
		case "container":
			...
			return ContainerElement{Configuration: configuration, Layout: layout, Children: children}, nil
		case "text":
			...
			return TextElement{Configuration: configuration, Value: value}, nil
		case "image":
			...
			return ImageElement{Configuration: configuration, Source: source, AlternativeText: alternativeText}, nil
		case "button":
			...
			return ButtonElement{Configuration: configuration, Children: children}, nil
		case "repeat":
			...
			return RepeatElement{Collection: collection, ItemName: wire.ItemName, IndexName: wire.IndexName, Key: key, Body: body}, nil
		case "conditional":
			...
			return ConditionalElement{Condition: condition, Then: then, Else: elseElement}, nil
		default:
