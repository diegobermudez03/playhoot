...
func encodeTypeDeclarationSlice(path string, items []TypeDeclaration) ([]json.RawMessage, error) {
	if items == nil {
		return nil, nil
	}
...
func decodeTypeDeclarationSlice(path string, items []json.RawMessage) ([]TypeDeclaration, error) {
	if items == nil {
		return nil, nil
	}
	result := make([]TypeDeclaration, len(items))
...
// json.RawMessage because TypeDeclaration is a closed interface whose
...
// root_workflow, workflows. Metadata and GlobalState always encode as
// ordinary objects, never JSON null.
func encodeDefinition(path string, value Definition) (json.RawMessage, error) {
...
// decodeDefinition decodes data as a Definition. JSON null is not a valid
// encoding of the root definition and produces a path-aware structural
// error at path; Metadata and GlobalState must always be ordinary objects
// (never null).
func decodeDefinition(path string, data json.RawMessage) (Definition, error) {
