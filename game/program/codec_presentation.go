...
func encodePresentationSlotDeclaration(path string, value PresentationSlotDeclaration) (json.RawMessage, error) {
	return json.Marshal(wirePresentationSlotDeclaration{Name: value.Name})
}

// decodePresentationSlotDeclaration decodes data as a
// PresentationSlotDeclaration. JSON null is not a valid encoding and
// produces a path-aware structural error.
func decodePresentationSlotDeclaration(path string, data json.RawMessage) (PresentationSlotDeclaration, error) {
	var wire wirePresentationSlotDeclaration
	if err := decodeOrdinaryObject(path, data, &wire); err != nil {
		return PresentationSlotDeclaration{}, err
	}
	return PresentationSlotDeclaration{Name: wire.Name}, nil
}

func encodePresentationSlotDeclarations(path string, items []PresentationSlotDeclaration) ([]json.RawMessage, error) {
	if items == nil {
		return nil, nil
	}
...
func decodePresentationSlotDeclarations(path string, items []json.RawMessage) ([]PresentationSlotDeclaration, error) {
	if items == nil {
		return nil, nil
	}
	result := make([]PresentationSlotDeclaration, len(items))
...
func encodePresentationDeclaration(path string, value PresentationDeclaration) (json.RawMessage, error) {
...
// decodePresentationDeclaration decodes data as a PresentationDeclaration.
// JSON null is not a valid encoding and produces a path-aware structural
// error.
func decodePresentationDeclaration(path string, data json.RawMessage) (PresentationDeclaration, error) {
	var wire wirePresentationDeclaration
	if err := decodeOrdinaryObject(path, data, &wire); err != nil {
		return PresentationDeclaration{}, err
	}
	...
	return PresentationDeclaration{
		Name:                wire.Name,
		Slot:                wire.Slot,
		Targets:             targets,
...
func encodePresentationDeclarations(path string, items []PresentationDeclaration) ([]json.RawMessage, error) {
	if items == nil {
		return nil, nil
	}
	result := make([]json.RawMessage, len(items))
...
func decodePresentationDeclarations(path string, items []json.RawMessage) ([]PresentationDeclaration, error) {
	if items == nil {
		return nil, nil
	}
	result := make([]PresentationDeclaration, len(items))
	for i, raw := range items {
...
// --- QuestionPresentationDeclaration (pointer helper) ---
...
// concrete pointer type directly because QuestionSlotDeclaration and
// AskGroupSlotDeclaration both use *QuestionPresentationDeclaration to
// represent "no authored presentation" with nil, not a closed interface.
func encodeQuestionPresentationDeclaration(path string, value *QuestionPresentationDeclaration) (json.RawMessage, error) {
	if value == nil {
...
// *QuestionPresentationDeclaration. Unlike other ordinary-object decoders
...
func decodeQuestionPresentationDeclaration(path string, data json.RawMessage) (*QuestionPresentationDeclaration, error) {
	...
	return &QuestionPresentationDeclaration{
		Slot:                wire.Slot,
		Projection:          wire.Projection,
		ProjectionArguments: arguments,
		View:                wire.View,
