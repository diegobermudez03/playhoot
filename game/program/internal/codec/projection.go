...
func encodeProjectionDeclaration(path string, value program.ProjectionDeclaration) (json.RawMessage, error) {
...
// decodeProjectionDeclaration decodes data as a program.ProjectionDeclaration.
// JSON null is not a valid encoding and produces a path-aware structural
// error.
func decodeProjectionDeclaration(path string, data json.RawMessage) (program.ProjectionDeclaration, error) {
	var wire wireProjectionDeclaration
	if err := decodeOrdinaryObject(path, data, &wire); err != nil {
		return program.ProjectionDeclaration{}, err
	}
	...
	return program.ProjectionDeclaration{Name: wire.Name, Parameters: parameters, ResultType: resultType, Body: body}, nil
}

func encodeProjectionDeclarations(path string, items []program.ProjectionDeclaration) ([]json.RawMessage, error) {
	if items == nil {
		return nil, nil
	}
...
func decodeProjectionDeclarations(path string, items []json.RawMessage) ([]program.ProjectionDeclaration, error) {
	if items == nil {
		return nil, nil
	}
	result := make([]program.ProjectionDeclaration, len(items))
