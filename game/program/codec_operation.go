...
// --- Block (ordinary struct, not a closed interface) ---

type wireBlock struct {
	Operations []json.RawMessage `json:"operations"`
}

func encodeOperationSlice(path string, items []Operation) ([]json.RawMessage, error) {
	if items == nil {
		return nil, nil
	}
...
func decodeOperationSlice(path string, items []json.RawMessage) ([]Operation, error) {
	if items == nil {
		return nil, nil
	}
	result := make([]Operation, len(items))
...
func encodeBlock(path string, block Block) (json.RawMessage, error) {
...
// decodeBlock decodes data as a Block. Because Block is an ordinary struct
...
// value.
func decodeBlock(path string, data json.RawMessage) (Block, error) {
	if isEmptyOrNull(data) {
		return Block{}, newDecodeError(path, "expected a block object, got null or missing value", nil)
	}
	...
	return Block{Operations: operations}, nil
}

// --- wire structs for every Operation variant ---
...
func encodeMatchOperationCases(path string, cases []MatchOperationCase) ([]wireMatchOperationCase, error) {
	if cases == nil {
		return nil, nil
	}
...
func decodeMatchOperationCases(path string, cases []wireMatchOperationCase) ([]MatchOperationCase, error) {
	if cases == nil {
		return nil, nil
	}
	result := make([]MatchOperationCase, len(cases))
	for i, c := range cases {
		itemPath := pathIndex(path, i)
		...
		result[i] = MatchOperationCase{Pattern: pattern, Body: body}
	}
	return result, nil
}
...
func encodeOperation(path string, value Operation) (json.RawMessage, error) {
	return encodeUnion(value, func(resolved any) (json.RawMessage, error) {
		...
		default:
			return nil, fmt.Errorf("%s: unsupported Operation implementation %T", path, value)
		}
	})
}

// decodeOperation decodes data as an Operation, or returns a nil interface
// for JSON null or a missing value.
func decodeOperation(path string, data json.RawMessage) (Operation, error) {
	return decodeUnion(path, data, func(path, kind string, raw json.RawMessage) (Operation, error) {
		switch kind {
		case "let":
			...
			return LetOperation{Name: wire.Name, Type: typeRef, Value: value}, nil
		case "set":
			...
			return SetOperation{Target: target, Value: value}, nil
		case "list_append":
			...
			return ListAppendOperation{Target: target, Value: value}, nil
		case "list_insert":
			...
			return ListInsertOperation{Target: target, Index: index, Value: value}, nil
		case "list_remove_at":
			...
			return ListRemoveAtOperation{Target: target, Index: index}, nil
		case "map_put":
			...
			return MapPutOperation{Target: target, Key: key, Value: value}, nil
		case "map_delete":
			...
			return MapDeleteOperation{Target: target, Key: key}, nil
		case "if":
			...
			return IfOperation{Condition: condition, Then: then, Else: elseBlock}, nil
		case "for_each":
			...
			return ForEachOperation{Collection: collection, ItemName: wire.ItemName, IndexName: wire.IndexName, Body: body}, nil
		case "match":
			...
			return MatchOperation{Value: matchValue, Cases: cases}, nil
		case "open_question":
			...
			return OpenQuestionOperation{Slot: wire.Slot, Recipient: recipient, Arguments: arguments}, nil
		case "close_question":
			var wire wireCloseQuestionOperation
			if err := strictDecodeInto(path, raw, &wire); err != nil {
				return nil, err
			}
			return CloseQuestionOperation{Slot: wire.Slot}, nil
		case "emit_effect":
			...
			return EmitEffectOperation{Effect: wire.Effect, Recipients: recipients, Arguments: arguments}, nil
		case "schedule_timer":
			...
			return ScheduleTimerOperation{Slot: wire.Slot, DelayMilliseconds: delay}, nil
		case "cancel_timer":
			var wire wireCancelTimerOperation
			if err := strictDecodeInto(path, raw, &wire); err != nil {
				return nil, err
			}
			return CancelTimerOperation{Slot: wire.Slot}, nil
		case "spawn_child_workflow":
			...
			return SpawnChildWorkflowOperation{Slot: wire.Slot, Arguments: arguments}, nil
		case "cancel_child_workflow":
			...
			return CancelChildWorkflowOperation{Slot: wire.Slot, Reason: reason}, nil
		case "open_ask_group":
			...
			return OpenAskGroupOperation{Slot: wire.Slot, Recipients: recipients, Arguments: arguments, Completion: completion}, nil
		case "finalize_ask_group":
			var wire wireFinalizeAskGroupOperation
			if err := strictDecodeInto(path, raw, &wire); err != nil {
				return nil, err
			}
			return FinalizeAskGroupOperation{Slot: wire.Slot}, nil
		case "cancel_ask_group":
			var wire wireCancelAskGroupOperation
			if err := strictDecodeInto(path, raw, &wire); err != nil {
				return nil, err
			}
			return CancelAskGroupOperation{Slot: wire.Slot}, nil
		case "begin_task_group":
			...
			return BeginTaskGroupOperation{Slot: wire.Slot, Completion: completion}, nil
		case "spawn_task_group_child":
			...
			return SpawnTaskGroupChildOperation{Slot: wire.Slot, Key: key, Arguments: arguments}, nil
		case "seal_task_group":
			var wire wireSealTaskGroupOperation
			if err := strictDecodeInto(path, raw, &wire); err != nil {
				return nil, err
			}
			return SealTaskGroupOperation{Slot: wire.Slot}, nil
		case "finalize_task_group":
			var wire wireFinalizeTaskGroupOperation
			if err := strictDecodeInto(path, raw, &wire); err != nil {
				return nil, err
			}
			return FinalizeTaskGroupOperation{Slot: wire.Slot}, nil
		case "cancel_task_group":
			...
			return CancelTaskGroupOperation{Slot: wire.Slot, Reason: reason}, nil
		case "draw_random":
			...
			return DrawRandomOperation{Name: wire.Name, Generator: generator}, nil
		default:
