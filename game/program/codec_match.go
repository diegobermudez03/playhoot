...
func encodeMatchPattern(path string, value MatchPattern) (json.RawMessage, error) {
	return encodeUnion(value, func(resolved any) (json.RawMessage, error) {
		...
		case EnumValueMatchPattern:
			return json.Marshal(wireEnumValueMatchPattern{
				Kind:      "enum_value",
				TypeName:  v.TypeName,
				ValueName: v.ValueName,
			})
		case UnionVariantMatchPattern:
		...
		case OptionalNoneMatchPattern:
			return json.Marshal(wireOptionalNoneMatchPattern{Kind: "optional_none"})
		case OptionalSomeMatchPattern:
			return json.Marshal(wireOptionalSomeMatchPattern{Kind: "optional_some", Binding: v.Binding})
		default:
			return nil, fmt.Errorf("%s: unsupported MatchPattern implementation %T", path, value)
		}
	})
}

// decodeMatchPattern decodes data as a MatchPattern, or returns a nil
// interface for JSON null or a missing value.
func decodeMatchPattern(path string, data json.RawMessage) (MatchPattern, error) {
	return decodeUnion(path, data, func(path, kind string, raw json.RawMessage) (MatchPattern, error) {
		switch kind {
		case "wildcard":
			...
			return WildcardMatchPattern{}, nil
		case "enum_value":
			var wire wireEnumValueMatchPattern
			if err := strictDecodeInto(path, raw, &wire); err != nil {
				return nil, err
			}
			return EnumValueMatchPattern{TypeName: wire.TypeName, ValueName: wire.ValueName}, nil
		case "union_variant":
			var wire wireUnionVariantMatchPattern
			if err := strictDecodeInto(path, raw, &wire); err != nil {
				return nil, err
			}
			var bindings []MatchFieldBinding
			if wire.Bindings != nil {
				bindings = make([]MatchFieldBinding, len(wire.Bindings))
				for i, b := range wire.Bindings {
					bindings[i] = MatchFieldBinding{Field: b.Field, Name: b.Name}
				}
			}
			return UnionVariantMatchPattern{
		...
		case "optional_none":
			...
			return OptionalNoneMatchPattern{}, nil
		case "optional_some":
			var wire wireOptionalSomeMatchPattern
			if err := strictDecodeInto(path, raw, &wire); err != nil {
				return nil, err
			}
			return OptionalSomeMatchPattern{Binding: wire.Binding}, nil
		default:
