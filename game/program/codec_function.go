...
func encodeFunctionDeclaration(path string, value FunctionDeclaration) (json.RawMessage, error) {
...
// decodeFunctionDeclaration decodes data as a FunctionDeclaration. JSON
// null is not a valid encoding and produces a path-aware structural error.
func decodeFunctionDeclaration(path string, data json.RawMessage) (FunctionDeclaration, error) {
	...
	return FunctionDeclaration{Name: wire.Name, Parameters: parameters, ResultType: resultType, Body: body}, nil
}

func encodeFunctionDeclarations(path string, items []FunctionDeclaration) ([]json.RawMessage, error) {
	if items == nil {
		return nil, nil
	}
...
func decodeFunctionDeclarations(path string, items []json.RawMessage) ([]FunctionDeclaration, error) {
	if items == nil {
		return nil, nil
	}
	result := make([]FunctionDeclaration, len(items))
...
// struct, as its JSON wire representation.
func encodeInvariantDeclaration(path string, value InvariantDeclaration) (json.RawMessage, error) {
...
// decodeInvariantDeclaration decodes data as an InvariantDeclaration. JSON
// null is not a valid encoding and produces a path-aware structural error.
func decodeInvariantDeclaration(path string, data json.RawMessage) (InvariantDeclaration, error) {
	var wire wireInvariantDeclaration
	if err := decodeOrdinaryObject(path, data, &wire); err != nil {
		return InvariantDeclaration{}, err
	}
	condition, err := decodeExpression(pathField(path, "condition"), wire.Condition)
	if err != nil {
		return InvariantDeclaration{}, err
	}
	return InvariantDeclaration{Name: wire.Name, Condition: condition}, nil
}

func encodeInvariantDeclarations(path string, items []InvariantDeclaration) ([]json.RawMessage, error) {
	if items == nil {
		return nil, nil
	}
	result := make([]json.RawMessage, len(items))
...
func decodeInvariantDeclarations(path string, items []json.RawMessage) ([]InvariantDeclaration, error) {
	if items == nil {
		return nil, nil
	}
	result := make([]InvariantDeclaration, len(items))
	for i, raw := range items {
