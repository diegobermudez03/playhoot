...
func TestValidate_EmptyDefinition_NoErrors(t *testing.T) {
	errs := program.Definition{}.Validate()
	if len(errs) != 0 {
...
func TestValidate_ArithmeticOnBoolLiteral_IsReported(t *testing.T) {
	...
	for _, err := range errs {
		if ve, ok := err.(*program.ValidationError); ok && ve.Path == "$.invariants[0].condition.right" {
			found = true
		}
	}
	if !found {
...
func TestValidate_ErrorsAreValidationErrorType(t *testing.T) {
	...
	for _, err := range errs {
		if _, ok := err.(*program.ValidationError); !ok {
			t.Fatalf("expected *program.ValidationError, got %T", err)
		}
	}
}
