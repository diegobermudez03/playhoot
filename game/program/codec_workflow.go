...
func encodeTransitionDeclaration(path string, value TransitionDeclaration) (json.RawMessage, error) {
...
// decodeTransitionDeclaration decodes data as a TransitionDeclaration.
...
// independently be JSON null.
func decodeTransitionDeclaration(path string, data json.RawMessage) (TransitionDeclaration, error) {
	var wire wireTransitionDeclaration
	if err := decodeOrdinaryObject(path, data, &wire); err != nil {
		return TransitionDeclaration{}, err
	}
...
func encodeTransitionDeclarations(path string, items []TransitionDeclaration) ([]json.RawMessage, error) {
	if items == nil {
		return nil, nil
	}
...
func decodeTransitionDeclarations(path string, items []json.RawMessage) ([]TransitionDeclaration, error) {
	if items == nil {
		return nil, nil
	}
	result := make([]TransitionDeclaration, len(items))
...
// and signal-driven.
func encodeWorkflowStateDeclaration(path string, value WorkflowStateDeclaration) (json.RawMessage, error) {
...
// WorkflowStateDeclaration. JSON null is not a valid encoding and produces
// a path-aware structural error.
func decodeWorkflowStateDeclaration(path string, data json.RawMessage) (WorkflowStateDeclaration, error) {
	var wire wireWorkflowStateDeclaration
	if err := decodeOrdinaryObject(path, data, &wire); err != nil {
		return WorkflowStateDeclaration{}, err
	}
	...
	return WorkflowStateDeclaration{Name: wire.Name, Presentations: presentations, Transitions: transitions}, nil
}

func encodeWorkflowStateDeclarations(path string, items []WorkflowStateDeclaration) ([]json.RawMessage, error) {
	if items == nil {
		return nil, nil
	}
	result := make([]json.RawMessage, len(items))
...
func decodeWorkflowStateDeclarations(path string, items []json.RawMessage) ([]WorkflowStateDeclaration, error) {
	if items == nil {
		return nil, nil
	}
	result := make([]WorkflowStateDeclaration, len(items))
	for i, raw := range items {
...
func encodeWorkflowDeclaration(path string, value WorkflowDeclaration) (json.RawMessage, error) {
...
// decodeWorkflowDeclaration decodes data as a WorkflowDeclaration. JSON
...
func decodeWorkflowDeclaration(path string, data json.RawMessage) (WorkflowDeclaration, error) {
	var wire wireWorkflowDeclaration
	if err := decodeOrdinaryObject(path, data, &wire); err != nil {
		return WorkflowDeclaration{}, err
	}
...
func encodeWorkflowDeclarations(path string, items []WorkflowDeclaration) ([]json.RawMessage, error) {
	if items == nil {
		return nil, nil
	}
	result := make([]json.RawMessage, len(items))
...
func decodeWorkflowDeclarations(path string, items []json.RawMessage) ([]WorkflowDeclaration, error) {
	if items == nil {
		return nil, nil
	}
	result := make([]WorkflowDeclaration, len(items))
	for i, raw := range items {
