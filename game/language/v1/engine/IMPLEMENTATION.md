...
- `step.go` — `ExecutionErrorCode`/`ExecutionError`, `NewSnapshot`, `Step` itself (transition selection, guard evaluation, control application, invariant checking, commit assembly). Since exactly one workflow instance ever exists, `Step` operates on the Snapshot's root instance directly — there is no instance-tree address to resolve.
