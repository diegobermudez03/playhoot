...
The one function in `engineservice` that is **not** a pure wrapper is `CheckSnapshotCompatibility` (`codec.go`) — it has real logic of its own (checking the snapshot's root instance against the compiled `Program`, building its own `*ExecutionError`), which is why it's tested directly in `engineservice`'s own test suite rather than in `internal/codec`'s.
...
- `compile_workflows.go`, `compile_slots.go`, `compile_control.go`, `compile_ask_groups.go`, `compile_signals.go`, `compile_calls.go`, `compile_match.go`, `compile_operations.go`, `compile_expressions.go`, `compile_ui.go` — the workflow-compilation phase, split by concern (slot declarations, control-flow validation, ask groups, signal-pattern matching, call-argument binding, match patterns, operations, expressions, UI/projection bindings).
...
- `step.go` — `ExecutionErrorCode`/`ExecutionError`, `NewSnapshot`, `Step` itself (interaction-address resolution, transition selection, guard evaluation, control application, invariant checking, commit assembly). Since exactly one workflow instance ever exists, `Step` operates on the Snapshot's root instance directly — there is no instance-tree address to resolve.
- `execute.go` — `execContext` (one `Step` call's mutable working state: the target instance's slots, local state, accumulated outputs/internal signals, operation/budget counters, and the candidate `nextInteractionID` counter) and every `exec*` method that runs one compiled `Operation` against it.
- `interaction.go` — `resolvedInteraction`/`resolveInteractionID`: resolving a caller-supplied `engine.InteractionID` to the underlying slot/key/family it addresses, so `Step` can reuse the same Slot(+Key)-parameterized validation/matching logic that predates `InteractionID`.
...
- `ask_group.go` — the ask-group structured-concurrency kind: its own operations, completion-policy resolution, and slot bookkeeping.
...
For a `SignalKindInteractionAnswered`/`SignalKindInteractionCompleted` signal, `Step`'s very first step is resolving `signal.InteractionID` (via `interaction.go`'s `resolveInteractionID`) to a local `resolvedInteraction` — the underlying slot, key (if any), and whether it is a Question or an Ask Group occurrence — building a local `resolved` copy of the signal with `Slot`/`Key` populated from that resolution, so every downstream helper (`validateQuestionAnswer`, `signalMatchesSource`, `signalSchemaFields`, and so on) is unchanged from before `InteractionID` existed, just parameterized by the resolved values instead of receiving them directly from the caller. An Ask Group occurrence's answer is then dispatched to `stepAskGroupAnswer`/`stepKeyedAskGroupAnswer` exactly as before; only how those two functions' `Slot`/`Key` arguments are obtained changed.

Inside `Step` (`step.go`), once a transition's guard passes and its operations have run without exceeding any `Limits`, every compiled `Invariant` is checked against the resulting global state — atomically; a violation aborts the whole `Step` with `ExecutionErrorInvariantViolation` and no `Commit` is produced. Only after that does the new target instance get built, and if this instance's own control just produced a terminal `RunOutcome`, its ordinary and keyed ask-group slots (running or terminal-awaiting-join) are discarded wholesale via `clearedAskGroupSlots`/`clearedKeyedAskGroupSlots`, and exactly one `engine.RunCompletedOutput` is appended to the commit's outputs — the one place a caller finds out the whole game instance ended.
...
3. Add runtime handling to `internal/runtime` (`execute.go` for a new `Operation`, `evaluate.go` for a new `Expression`, or a new file if it's a big enough concern — see `ask_group.go` as the precedent for "big enough to deserve its own file").
