...
func encodeRandomGenerator(path string, value program.RandomGenerator) (json.RawMessage, error) {
	return encodeUnion(value, func(resolved any) (json.RawMessage, error) {
		...
		default:
			return nil, fmt.Errorf("%s: unsupported program.RandomGenerator implementation %T", path, value)
		}
	})
}

// decodeRandomGenerator decodes data as a program.RandomGenerator, or returns a
// nil interface for JSON null or a missing value.
func decodeRandomGenerator(path string, data json.RawMessage) (program.RandomGenerator, error) {
	return decodeUnion(path, data, func(path, kind string, raw json.RawMessage) (program.RandomGenerator, error) {
		switch kind {
		case "random_integer":
			...
			return program.RandomIntegerGenerator{Minimum: minimum, Maximum: maximum}, nil
		case "random_element":
			...
			return program.RandomElementGenerator{Collection: collection}, nil
		case "random_shuffle":
			...
			return program.RandomShuffleGenerator{Collection: collection}, nil
		default:
