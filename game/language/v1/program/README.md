...
| `Operation`                 | one synchronous step inside a transition   | `LetOperation`, `SetOperation`, list/map mutations, `IfOperation`, `ForEachOperation`, question/ask-group/timer operations and their keyed families, `DrawRandomOperation`, `MatchOperation`                                                                                                                                                                                           |
...
| `SignalSource`              | what a transition reacts to                | `NamedSignalSource` (platform signals), `UserIntentSignalSource`, `QuestionAnsweredSignalSource`, `TimerExpiredSignalSource`, `AskGroupCompletedSignalSource`, `KeyedQuestionAnsweredSignalSource`, `KeyedTimerExpiredSignalSource`, `KeyedAskGroupCompletedSignalSource`                                                                                                  |
...
`UserDisconnected` and `UserReconnected` are accepted as standard `NamedSignalSource` platform/lifecycle signals — the same mechanism as `WorkflowStarted`/`SessionCancelled` — each exposing exactly one authored field, `user: user` (the Session-local runtime identity derived from `SessionActorID`, never `Identity.UserUUID` or any Coordinator/transport internal). Session Runtime delivers both to the one workflow instance a game runs as. Handling either signal is optional; an authored game with no matching transition experiences an ordinary rejected/unmatched signal with no automatic gameplay consequence.
...
Question, Ask Group, and Timer slots each have a keyed family — `KeyedQuestionSlotDeclaration`, `KeyedAskGroupSlotDeclaration`, `KeyedTimerSlotDeclaration` — generalizing their ordinary single-occurrence counterparts to hold several independent, simultaneously pending occurrences at once, addressed by an authored key (typically a player, team, or object identity) instead of at most one occurrence per slot.

Conceptual identity for a keyed slot is `(slot, key)`: at most one occurrence may be pending per exact tuple, but different keys under the same slot are fully independent and may be simultaneously pending (for example `disconnect_timeout[P1]` and `disconnect_timeout[P2]` pending at once). Opening/scheduling into an already-occupied `(slot, key)` is an execution error, atomic for the whole enclosing transition, with no implicit reset/replace/coalesce — mirroring the corresponding ordinary slot's own occupied-slot rule exactly, just scoped to one key. The key is authored-language information, exposed to whatever resolves against it (a matched transition's bound `"key"` field, and — for Question — a mounted presentation's projection arguments) rather than an internal addressing detail. The engine's one-active-presentation-per-slot-per-user rule still applies per user, not per key: two simultaneously pending keyed occurrences targeting the same recipient and the same presentation slot is an execution error, same as for two ordinary presentations.

Every existing `AskGroupCompletionPolicy` variant is reused unchanged for `KeyedAskGroupSlotDeclaration`, evaluated independently per `(slot, key)` occurrence. `QuestionPresentationDeclaration` is likewise reused unchanged as the presentation config type for keyed Question/AskGroup slots — though it is currently only ever mounted for keyed Question, since the future engine does not yet derive any presentation for an ordinary `AskGroupSlotDeclaration.Presentation` either; a keyed ask group's configured `Presentation` compiles but has no current runtime mounting path, matching that pre-existing, unrelated gap in the ordinary family it mirrors.
...
Several concepts are modeled as "slots" — durable, statically named locations owned by a workflow instance that hold something in-flight: `QuestionSlotDeclaration` (a pending question), `AskGroupSlotDeclaration` (a pending multi-recipient ask), `TimerSlotDeclaration` (a pending timer). A slot name is always a static, source-level string — never a runtime expression. Each of these three has a keyed family (`KeyedQuestionSlotDeclaration`/`KeyedAskGroupSlotDeclaration`/`KeyedTimerSlotDeclaration`) holding several independent occurrences at once, addressed by an authored key — see "Keyed Interaction Slots" above.
