...
| `Operation`                 | one synchronous step inside a transition   | `LetOperation`, `SetOperation`, list/map mutations, `IfOperation`, `ForEachOperation`, question/ask-group/timer operations, `DrawRandomOperation`, `MatchOperation`                                                                                                                                                                                           |
...
| `SignalSource`              | what a transition reacts to                | `NamedSignalSource` (platform signals), `UserIntentSignalSource`, `QuestionAnsweredSignalSource`, `TimerExpiredSignalSource`, `AskGroupCompletedSignalSource`                                                                                                  |
...
Several concepts are modeled as "slots" — durable, statically named locations owned by a workflow instance that hold something in-flight: `QuestionSlotDeclaration` (a pending question), `AskGroupSlotDeclaration` (a pending multi-recipient ask), `TimerSlotDeclaration` (a pending timer). A slot name is always a static, source-level string — never a runtime expression.
