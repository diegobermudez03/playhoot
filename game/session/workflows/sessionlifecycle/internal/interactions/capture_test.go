...
// fakeCaptureRepo call, for assertions.
...
func (f *fakeCaptureRepo) CreateInteraction(ctx context.Context, tx *gorm.DB, sessionID uint, sessionActorID uint, kind string, engineInteractionID uint64, interactionPayload []byte, openedByTurnID uint) (uint, error) {
...
func (f *fakeCaptureRepo) CloseActiveInteraction(ctx context.Context, tx *gorm.DB, sessionID uint, engineInteractionID uint64, sessionActorID uint, closedByTurnID uint) error {
...
func TestCapture(t *testing.T) {
	...
	t.Run("open_then_close_in_one_turn_use_the_same_interaction_id", func(t *testing.T) {
		repo := &fakeCaptureRepo{}
		outputs := []engine.Output{
		...
		require.Len(t, repo.creates, 1)
		require.Len(t, repo.closes, 1)
		require.Equal(t, repo.creates[0].engineInteractionID, repo.closes[0].engineInteractionID, "both outputs came from the same Turn, addressing the same occurrence")
	})
