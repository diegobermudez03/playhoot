...
func TestCaptureInteractions(t *testing.T) {
	t.Run("open_question_output_creates_an_active_question_interaction", func(t *testing.T) {
		...
		err := captureInteractions(context.Background(), nil, repo, 1, 10, outputs)
	...
	t.Run("close_question_output_closes_the_matching_active_interaction", func(t *testing.T) {
		...
		err := captureInteractions(context.Background(), nil, repo, 1, 11, outputs)
		require.NoError(t, err)
	...
	t.Run("open_then_close_in_one_turn_use_the_same_interaction_id", func(t *testing.T) {
		...
		err := captureInteractions(context.Background(), nil, repo, 1, 12, outputs)
		require.NoError(t, err)
		require.Len(t, repo.creates, 1)
		require.Len(t, repo.closes, 1)
	...
	t.Run("no_outputs_is_a_no_op", func(t *testing.T) {
		...
		err := captureInteractions(context.Background(), nil, repo, 1, 13, nil)
		require.NoError(t, err)
		require.Empty(t, repo.creates)
		require.Empty(t, repo.closes)
	})
