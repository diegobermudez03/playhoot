...
// Create/Join/Leave/Start (see step_create.go/step_join.go/step_leave.go/
// step_start.go) as its steps. Each step depends on its own narrow
// persistence contract (createRepoAPI/joinRepoAPI/leaveRepoAPI/startRepoAPI)
// even though, today, one concrete internal/repo.Repo satisfies all four
...
// RuntimeTurn execution logic (Step-draining, the 20-Step bound, Turn/Step/
// State persistence) lives inline inside step_start.go rather than a
// separate shared package - per explicit human direction (2026-09-19, see
// WORK-0003) - since no other current use case calls it independently.
//
// Manager decides transaction scope itself by calling
// utils.RunInDBTransaction directly (see step_create.go/step_join.go/
// step_leave.go/step_start.go), rather than holding a separate injected
...
// "current version" (Join's dependency, reused unchanged by Start - see
// WORK-0001's Pinned Game Definition Is Immutable For The Session).
