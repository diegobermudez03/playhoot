...
Total loss of ephemeral Coordinator/process state does not itself mutate `session_actors.semantic_presence` (GAME-ADR-0015): durable `CONNECTED`/`DISCONNECTED` values before the loss remain unchanged merely because sockets, Coordinator maps, timers, or grace state disappeared with the crashed process. `semantic_presence = CONNECTED` means only "Session Runtime has not yet processed a semantic `CONNECTED -> DISCONNECTED` edge for this actor," not that a live socket currently exists - physical connectivity remains entirely Coordinator-owned ephemeral state. When Coordinator reconstructs a still-semantically-active Session and finds a durably `CONNECTED` actor with no current physical binding, it starts a fresh transport/recovery grace for that actor, reusing the same mechanism already accepted for ordinary transient disconnects (GAME-ADR-0010) rather than a distinct process-crash-disconnect event or API; a durably `DISCONNECTED` actor receives no such grace, since there is no semantic uncertainty to absorb. V1 does not persist elapsed/remaining physical-disconnect grace time, so a restarted Coordinator may restart the full configured grace duration for a durably `CONNECTED` actor with no binding - the same simplicity tradeoff already accepted for Game Language timer obligations (GAME-ADR-0008), applied to a distinct mechanism. Recovery grace must never revive a Session whose authoritative `lobby_expires_at`/`activity_expires_at` deadline already passed, and merely reconstructing infrastructure mechanisms after restart (loading a Snapshot, discovering timer obligations/durably `CONNECTED` actors, starting recovery graces, rebuilding Coordinator maps) does not by itself renew `activity_expires_at`. Session Runtime does not gain process-specific APIs (for example `RecoverFromProcessCrash`, `ClaimSession`, `CoordinatorRestarted`) - it continues to expose only ordinary domain/runtime operations regardless of which process invokes them.

For a RUNNING runtime member, the durable `semantic_presence` transition and its corresponding Game Language lifecycle processing (`UserDisconnected`/`UserReconnected`) commit as a single Session transaction, extending the existing RuntimeTurn transactional model to the presence mutation itself: the transition, the signal delivery attempt, and any resulting RuntimeTurn/consequences (or, if the signal is unhandled, no RuntimeTurn at all, per GAME-ADR-0011) commit together or not at all. If the process crashes before that transaction commits, the presence edge did not occur authoritatively and durable state remains at its prior value; if it committed, presence and any Game Language effect are already authoritative together, so a crash can never durably record a semantic-presence transition while silently losing the corresponding authored lifecycle effect.

Rationale and alternatives, including the rejected process-owned-lease/heartbeat/fencing direction, are recorded in `game/docs/decisions/GAME-ADR-0013-session-runtime-process-agnostic-recovery.md`. The recovery-grace-after-total-process-loss behavior and the RUNNING atomic semantic-presence/Game-Language-processing rule above are recorded in `game/docs/decisions/GAME-ADR-0016-session-semantic-presence-recovery-after-total-coordinator-state-loss.md`.
...
Merely reconstructing infrastructure mechanisms after a Coordinator/process restart - scanning `RUNNING` Sessions, loading a current Snapshot, discovering active `session_timer_obligations`, rescheduling physical timers, discovering durably `CONNECTED` actors (see Session Runtime Process-Agnostic Recovery above), starting a Coordinator recovery grace, or rebuilding Coordinator maps - does not by itself renew `activity_expires_at`; otherwise infrastructure restarts could keep an abandoned Session alive indefinitely. A later real operation, such as an authenticated reconnect, may separately qualify as meaningful activity under the renewal policy above. See `game/docs/decisions/GAME-ADR-0016-session-semantic-presence-recovery-after-total-coordinator-state-loss.md`.
...
Total loss of Coordinator/process state (a crash, a redeploy) does not itself mutate `semantic_presence`; durable `CONNECTED`/`DISCONNECTED` values survive unchanged, since `CONNECTED` already means only "no semantic disconnect edge processed yet," not "a live socket exists right now" (see Session Runtime Process-Agnostic Recovery above). When a new Coordinator process reconstructs a still-semantically-active Session and finds a durably `CONNECTED` actor with no current physical binding, it starts a fresh transport/recovery grace for that actor using this same grace mechanism - not a distinct process-crash-disconnect event. If the actor rebinds before that fresh grace expires, the SessionActor remains `CONNECTED`, no `UserDisconnected`/`UserReconnected` is emitted, and Coordinator simply resends normal resync state - this is what prevents an ordinary process restart/deployment from producing artificial disconnect/reconnect pairs for every still-connected player. If the fresh grace expires without a binding, Coordinator invokes this same ordinary semantic-disconnect path described above, and the already-accepted phase-dependent LOBBY/RUNNING rules apply unchanged. A durably `DISCONNECTED` actor receives no recovery grace, since there is no semantic uncertainty to absorb; a later connection uses the ordinary reconnect path above. Recovery grace must never revive a Session whose `lobby_expires_at`/`activity_expires_at` deadline already passed (see Session Runtime Lobby Lifecycle Contract and Session Runtime Durable Inactivity Expiration above), and V1 restarts the full configured grace duration after process loss rather than persisting elapsed/remaining grace time.

For a RUNNING runtime member, the `semantic_presence` transition and the corresponding `UserDisconnected`/`UserReconnected` Game Language processing commit as one Session transaction, so a crash can never durably record the presence edge while losing the corresponding authored lifecycle effect (or the reverse); an unhandled signal still commits the presence transition alone, consistent with the no-op RuntimeTurn rule below.
...
Rationale and alternatives are recorded in `game/docs/decisions/GAME-ADR-0010-session-disconnect-reconnect-resync-boundary.md`. The durable `session_actors.semantic_presence` field and the LOBBY-vs-RUNNING phase-dependent participation consequence above are recorded in `game/docs/decisions/GAME-ADR-0015-session-actor-semantic-presence-and-lobby-membership.md`. The recovery-grace-after-total-process-loss behavior and the RUNNING atomic semantic-presence/Game-Language-processing rule are recorded in `game/docs/decisions/GAME-ADR-0016-session-semantic-presence-recovery-after-total-coordinator-state-loss.md`.
