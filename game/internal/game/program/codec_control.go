...
func encodeMatchControlCases(path string, cases []MatchControlCase) ([]wireMatchControlCase, error) {
	if cases == nil {
		return nil, nil
	}
...
func decodeMatchControlCases(path string, cases []wireMatchControlCase) ([]MatchControlCase, error) {
	if cases == nil {
		return nil, nil
	}
	result := make([]MatchControlCase, len(cases))
	for i, c := range cases {
		itemPath := pathIndex(path, i)
		...
		result[i] = MatchControlCase{Pattern: pattern, Control: control}
	}
	return result, nil
}
...
func encodeWorkflowControl(path string, value WorkflowControl) (json.RawMessage, error) {
	return encodeUnion(value, func(resolved any) (json.RawMessage, error) {
		...
		default:
			return nil, fmt.Errorf("%s: unsupported WorkflowControl implementation %T", path, value)
		}
	})
}

// decodeWorkflowControl decodes data as a WorkflowControl, or returns a
// nil interface for JSON null or a missing value.
func decodeWorkflowControl(path string, data json.RawMessage) (WorkflowControl, error) {
	return decodeUnion(path, data, func(path, kind string, raw json.RawMessage) (WorkflowControl, error) {
		switch kind {
		case "goto":
			...
			return GotoControl{State: wire.State}, nil
		case "stay":
			var wire wireStayControl
			if err := strictDecodeInto(path, raw, &wire); err != nil {
				return nil, err
			}
			return StayControl{}, nil
		case "complete":
			...
			return CompleteControl{Result: result}, nil
		case "fail":
			...
			return FailControl{Error: errExpr}, nil
		case "cancel":
			...
			return CancelControl{Reason: reason}, nil
		case "conditional":
			...
			return ConditionalControl{Condition: condition, Then: then, Else: elseControl}, nil
		case "match":
			...
			return MatchControl{Value: matchValue, Cases: cases}, nil
		default:
