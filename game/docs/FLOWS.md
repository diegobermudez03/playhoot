...
## Live Transport: Join / Start / AnswerInteraction Fan-Out (WebSocket)

A real client connects over WebSocket and exercises `Join`/`Start`/`AnswerInteraction` live, with the interaction Session Runtime opens delivered back to it and its answer delivered back to Session Runtime (WORK-0005). `Create` stays ordinary HTTP request/response - it happens before any Participant or connection exists. `Join` does not: it is the WebSocket handshake itself, so a client can never end up an active Participant without also being a bound live connection (or vice versa) - see this file's own history further down for why.
