        ...
        alt phase == RUNNING (a different token already started it)
            ...
            alt recompile fails (RUNTIME_STATE_INVALID)
                ...
                alt initialization fails or exceeds Step bound (RUNTIME_EXECUTION_FAILED)
                    ...
                    Mgr->>Repo: SetSessionRunning (phase, started_at, current_turn_id)
                    alt completion.Detect(outputs) finds a RunCompletedOutput
                        Mgr->>Repo: SetSessionTerminal(reason=GAME_COMPLETED/GAME_FAILED/GAME_CANCELLED) / CloseAllActiveInteractionsForSession
                        Repo->>DB: phase=TERMINAL (started_at stays set), terminal_reason, any other ACTIVE interaction closed
                    end
                    Mgr->>Repo: RevokeActiveJoinCode
                    Repo->>DB: Turn 1, Seed/RootParameters, join_codes revoked, any opened session_interactions
                    Mgr->>Idem: Complete(STARTED)
                    Mgr->>Repo: FindActorsByIDs (mapOutputs resolving each Output's recipient(s) to UserUUID)
                    Mgr-->>Caller: StartResult.Outputs = mapOutputs(clientoutputs.ClientFacing(outputs)), TerminalReason = completion.Detect's reason (empty if the game is still running)
                end
            end
        end
    end
...
- `clientoutputs.ClientFacing` (`internal/clientoutputs/clientoutputs.go`) selects the same Turn's `EmitEffectOutput`/`ActivatePresentationOutput`/`UpdatePresentationOutput`/`RemovePresentationOutput` values, in the same relative order - additive data only, computed fresh every call, never persisted (a future resync recomputes current Presentation state from the current Snapshot on demand instead of reading anything back). `(*Manager).mapOutputs` (`outputs.go`) then translates that selection into `sessionlifecycle`'s own exported `Output`/`Value` schema before `StartResult.Outputs` returns it: no exported `sessionlifecycle` symbol requires a caller to import `game/language/v1/engine`, and every recipient - the engine's internal actor-id-derived `UserID`, wherever it appears, including nested inside a Presentation's Model or an Effect's Arguments - is resolved to a real `UserUUID` via one batched `internal/repo.FindActorsByIDs` lookup per call. No caller outside this package consumes `Outputs` yet.
- `completion.Detect` (`internal/completion/completion.go`) scans the same Turn's `[]engine.Output` for an `engine.RunCompletedOutput` and, if found, maps its `Outcome.Kind` (`Completed`/`Failed`/`Cancelled`) to one of `session.TerminalReasonGameCompleted`/`GameFailed`/`GameCancelled` - three non-fatal reasons distinct from `RuntimeExecutionFailed`/`RuntimeStateInvalid` (an engine-execution malfunction, not a deliberate authored outcome) and from any future session-lifecycle-level cancellation. When found, the Session terminalizes in the same transaction as Turn 1 with `started_at` already set (the Session genuinely ran before ending) and any other still-`ACTIVE` interaction closes via `CloseAllActiveInteractionsForSession`, the same terminal cleanup the fatal path already performs. `RunOutcome`'s own payload (`Result`/`Error`/`Reason`) is not durably persisted - nothing reads it today, and it is re-derivable later by replaying the durable signal log if a future consumer needs it. The fact and reason are additively reported on `StartResult.TerminalReason`, so a future Coordinator learns the Session just ended without re-deriving it from `Outcome` alone.
- Behavior shared by two or more of this workflow's steps (lazy-expiration materialization, interaction capture, client-facing Output filtering, game-completion detection, replay reconstruction) lives under `sessionlifecycle`'s own `internal/<mechanism-name>/` subpackages (`internal/expiration/`, `internal/interactions/`, `internal/clientoutputs/`, `internal/completion/`, `internal/replay/`), sibling to `internal/repo/` - never inline in a `step_*.go` file, and never elevated to the domain-wide `game/session/internal/`.

Evidence:

- `game/session/workflows/sessionlifecycle/step_start.go`, `outputs.go`, `internal/replay/replay.go`, `internal/interactions/capture.go`, `internal/clientoutputs/clientoutputs.go`, `internal/completion/completion.go`, `internal/expiration/expiration.go`, `game/language/v1/engine/engineservice/runtime.go` (`StartTurn`/`AdvanceTurn`), `internal/repo/runtime_turn.go`, `internal/repo/runtime_start.go`, `internal/repo/interaction.go`, `internal/repo/session.go`'s `SetSessionRunning`/`SetSessionTerminal`, `internal/repo/participant.go`'s `ListActiveParticipantsForRoster`, `internal/repo/actor.go`'s `FindActorsByIDs`
        ...
        alt actor missing or not the interaction's recipient
            ...
            alt recompile fails (RUNTIME_STATE_INVALID)
                ...
                alt priorSignals failed to replay (ErrReplayDivergence)
                    ...
                    alt completion.Detect(outputs) finds a RunCompletedOutput
                        Mgr->>Repo: SetSessionTerminal(reason=GAME_COMPLETED/GAME_FAILED/GAME_CANCELLED) / CloseAllActiveInteractionsForSession
                        Repo->>DB: phase=TERMINAL, terminal_reason, any other ACTIVE interaction closed
                    end
                    Mgr->>Repo: FindActorsByIDs (mapOutputs resolving each Output's recipient(s) to UserUUID)
                    Mgr-->>Caller: AnswerInteractionOutcomeAnswered, Outputs = mapOutputs(clientoutputs.ClientFacing(outputs)), TerminalReason = completion.Detect's reason (empty if the game is still running)
                end
            end
        end
    end
...
- `AnswerInteractionResult.Outputs` carries the same accepted Turn's Effect/Presentation occurrences (`clientoutputs.ClientFacing(outputs)`, translated by `mapOutputs` into `sessionlifecycle`'s own `Output`/`Value` schema, in commit order) whenever `Outcome` is `Answered` and this call actually executed the engine; every declined/replayed/conflicting outcome leaves it empty, since no new RuntimeTurn committed.
- The same accepted Turn's `outputs` also feed `completion.Detect`, shared unchanged with `Start` - a `RunCompletedOutput` (the game's one instance reaching `Completed`/`Failed`/`Cancelled`) terminalizes the Session under one of `TerminalReasonGameCompleted`/`GameFailed`/`GameCancelled`, closing any other still-`ACTIVE` interaction via `CloseAllActiveInteractionsForSession` in the same transaction as the new RuntimeTurn - reported additively on `AnswerInteractionResult.TerminalReason`, distinct from the fatal-path `RuntimeExecutionFailed`/`RuntimeStateInvalid` reasons the `ErrReplayDivergence`/other-failure branches above already produce. `RunOutcome`'s own payload is not durably persisted.
...
- `game/session/workflows/sessionlifecycle/step_answer_interaction.go`, `outputs.go`, `internal/replay/replay.go`, `internal/interactions/capture.go`, `internal/clientoutputs/clientoutputs.go`, `internal/completion/completion.go`, `game/language/v1/engine/engineservice/runtime.go` (`AdvanceTurn`), `internal/repo/interaction.go`, `internal/repo/runtime_start.go`, `internal/repo/actor.go`'s `FindActorsByIDs`
