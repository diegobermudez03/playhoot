...
`docs/projects/active/game-language-flat-execution-model/works/WORK-0028-engine-owned-turn-execution.md` (via `game/docs/decisions/GAME-ADR-0027-engine-owned-turn-execution-and-replay.md`) generalizes this record's replay-first principle one step further: the mechanics of performing replay - not just the decision to never persist a Snapshot - move from `sessionlifecycle`'s own code into `engineservice.AdvanceTurn`, which now internally reconstructs current state from the same durable replay-input log this record already established. Nothing this record accepts about *what* is durably persisted changes.
