...
- The exact `closure_reason`/equivalent enum values for `session_timer_obligations` (Slice 5, not yet implemented), and the exact `runtime_turn_step_limit_exceeded`-equivalent stable error-code string (see GAME-ADR-0019). `MAX_STEPS_PER_RUNTIME_TURN` is implemented as a code-level constant (`internal/runtimeturn.MaxSteps = 20`); `session_interactions`' own `state`/`closure_reason` enum values (`ACTIVE | CLOSED | TERMINATED`, `SESSION_TERMINATED`) are implemented - see `game/docs/DATA_MODEL.md`.
