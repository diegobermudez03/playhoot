...
Status: **MATERIALIZED AS DRAFT WORK, not started.** `docs/work/active/WORK-0003-session-start-first-runtimeturn.md` was created 2026-09-19, reevaluating this slice against the actual post-Slice-1 codebase (including the `game/game` -> `game/management` rename, WORK-0002, DONE) and the fully-implemented Slice 1 Manager/sessionlock/idempotency pattern. The WORK proposes a new shared `game/session/internal/runtimeturn` mechanism package as the common RuntimeTurn executor (Step-draining, the 20-Step bound, Turn/Step/State persistence) that this slice introduces and Slices 3/5/6 reuse unchanged. **DRAFT, not READY** - it carries four explicit Blockers (implementation-planning gaps resolved with a proposed reading rather than silently decided) pending human confirmation; see its Blockers section and this workspace's `HUMAN_REVIEW.md`. No Slice 2 implementation has begun.
...
- **WORK-0003** - `docs/work/active/WORK-0003-session-start-first-runtimeturn.md` - Slice 2, Start + First RuntimeTurn. Status: **DRAFT** (created 2026-09-19). Not yet READY - see its Blockers section for four pending human-confirmation items. Implementation has not begun.
