...
Status: **DRAFT** (2026-09-19). `docs/work/active/WORK-0004-interaction-response-processing.md` materialized after reconciling this slice's design against the actual post-Slice-2 codebase. Carries five explicit Blockers awaiting human resolution: (1) whether interaction/timer/presence processing lives in a new sibling `game/session/workflows/runtimeexecution` package rather than on the existing `sessionlifecycle.Manager`; (2) extracting the shared RuntimeTurn Step-draining/bound execution logic (`game/session/internal/runtimeturn`) now, reversing Slice 2's explicit deferral now that a second caller needs it; (3) retrofitting `step_start.go` to capture interactions opened by its own first Turn - a real gap found during this design pass, since no Turn-producing code today persists `session_interactions` at all; (4) how `session_interactions.kind` (`QUESTION` vs `ASK_GROUP`) is determined for constructing the correct response signal; (5) narrow terminal-cleanup scope for this slice's own fatal path (closing any still-`ACTIVE` interaction it leaves behind) versus deferring entirely to Slice 6. See `HUMAN_REVIEW.md` for the human-facing summary.
...
- Slice 2 gates Slices 3, 5, and 6 (all RuntimeTurn-producing work follows its Step-draining/20-Step-guard pattern - there must be no window where any of them can drain `InternalSignals` unbounded; whether Slice 3/5 extract Slice 2's inline `step_start.go` logic into a shared package or reimplement it locally is decided when each materializes, per the human's 2026-09-19 rejection of introducing that shared package pre-emptively).
