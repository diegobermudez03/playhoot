...
Status: NOT COMPLETE - **READY** again as of 2026-09-16, not DONE. A first implementation pass (Create/Join/Leave) exists, builds, and passes its service-level tests, and the 2026-09-11 standard-compliance migration (move to `game/session/workflows/sessionlifecycle/...`; remove the horizontal `internal/actors` package) is done - but a 2026-09-16 handoff found a further service/repository/workflow-organization mismatch (three sibling packages instead of one `Manager` workflow controller, ceremonial params structs, repository methods deciding business/expiration/idempotency policy, state-based rather than token-aware Join idempotency), clarified the relevant reusable engineering standards and Session Join idempotency semantics (GAME-ADR-0021), and revised `docs/work/active/WORK-0001-session-lobby-foundation.md`'s approved design accordingly (see its "2026-09-16 Design Revision" section). WORK-0001 is **READY** again, not DONE; the existing implementation must still be migrated to the revised design before the remaining gates (real-Postgres integration/concurrency verification, independent review) can be meaningfully attempted. **Slice 2 has not started and must not start until WORK-0001 reaches DONE.** See `game/CURRENT_STATE.md` and `game/docs/FLOWS.md` for current *actual-code* reality (unaffected by this documentation-only revision) and `AI_CONTEXT.md`'s 2026-09-16 checkpoint for the full revision record.
...
Status: BLOCKED. The engineering-standard-compliance migration gate (`AI_CONTEXT.md` -> "Mandatory Standard-Compliance Migration") that used to block this slice is CLOSED (completed 2026-09-11), but a further 2026-09-16 design revision reopened WORK-0001 for a second, clarified standard-compliance migration (one `Manager` workflow controller, explicit-parameter methods, explicit transaction ownership, persistence-oriented repository naming, token-aware Join idempotency - see `WORK-0001`'s "2026-09-16 Design Revision" and `AI_CONTEXT.md`). This slice remains blocked on WORK-0001 reaching DONE: the 2026-09-16 implementation migration, real-Postgres integration/concurrency test verification, and independent implementation review. Not an architecture change to this slice's design below.
...
- **WORK-0001** - `docs/work/active/WORK-0001-session-lobby-foundation.md` - Slice 1, Session Lobby Foundation. Status: **READY** (as of the 2026-09-16 design revision; see its "2026-09-16 Design Revision" section). A first implementation pass (Create/Join/Leave) exists and its service-level tests pass, and the 2026-09-11 standard-compliance migration is done, but the implementation must now be migrated to the revised design (one `Manager` workflow controller, explicit-parameter methods, explicit transaction ownership, persistence-oriented repository naming, Manager-owned expiration/admission policy, token-aware Join idempotency) before DONE; also remaining: real-Postgres integration/concurrency test verification and independent implementation review.
...
**Slice 1 — Session Lobby Foundation** was correctly recommended and sequenced first; a first pass was implemented (Create/Join/Leave) as `WORK-0001-session-lobby-foundation.md`, with the mandatory 2026-09-11 standard-compliance migration also complete, but a 2026-09-16 handoff found the implementation still did not comply with clarified engineering standards and revised WORK-0001's design again - it is currently **READY** again (revised, re-approved), not DONE, pending a further implementation pass plus real-Postgres verification and independent review (see WORK References above and `AI_CONTEXT.md`). The human's implementation-design review that produced the original approved design added corrections around pinned-Game-Definition immutability, a minimum new Game Management read capability, the `(user_uuid, operation, idempotency_key)` idempotency namespace, concurrent-Create correctness, and the Session/HostActor insertion cycle; the 2026-09-16 revision further added the `Manager`-controller/explicit-parameter/transaction-ownership/repository-naming/token-aware-idempotency requirements - see WORK-0001 for the full corrected design.
