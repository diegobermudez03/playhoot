...
Status: **IMPLEMENTING.** `docs/work/active/WORK-0003-session-start-first-runtimeturn.md` was created DRAFT 2026-09-19, reevaluating this slice against the actual post-Slice-1 codebase (including the `game/game` -> `game/management` rename, WORK-0002, DONE) and the fully-implemented Slice 1 Manager/sessionlock/idempotency pattern; the human resolved all four Blockers the same day (see `HUMAN_REVIEW.md`), moving it to READY. **Material change from the original DRAFT proposal**: the human rejected introducing a new shared `game/session/internal/runtimeturn` mechanism package at this slice - the Step-draining/20-Step-bound/Turn-Step-State persistence logic instead lives inline inside `step_start.go` (workflow execution logic, not a capability another use case calls independently); a shared package remains deferred until Slice 3 actually needs to reuse this logic. No Slice 2 implementation has begun.

**Delivers**: Start transitions `LOBBY -> RUNNING` by calling `engineservice.NewSnapshot`, draining `Commit.InternalSignals` through repeated `Step` calls (this logic living inline in `step_start.go`, not a shared package - see above), and persisting the resulting Turn 1 / `session_runtime_state` / `session_runtime_turns` / `session_runtime_steps` atomically with `phase = RUNNING`, `started_at`, and JoinCode revocation. **This slice establishes the `MAX_STEPS_PER_RUNTIME_TURN = 20` bound from the moment RuntimeTurn execution first exists** - there must be no intermediate implementation where Start can drain `InternalSignals` indefinitely; Slices 3/5 will decide independently, when they materialize, whether to extract this logic into a shared package Start's implementation reuses, or implement their own equivalent. Includes the pre-first-Turn fatal-failure path (GAME-ADR-0019): a deterministic initialization failure (including exceeding the Step bound) terminalizes directly from `LOBBY` with `base_turn_id = NULL`, no Turn 1, `started_at` left null - distinguished from transient infrastructure failure, which leaves the Session retryable in `LOBBY`. The richer persistent `SessionRuntimeFailure` diagnostics surface (stable error codes, `diagnostic_payload`, querying failures by cause) is intentionally deferred to Slice 6 - this slice only needs the guard to actually stop execution and terminalize correctly, not the full diagnostic entity.
