...
`logging.Start` also attaches a trace ID to the request/connection: if the `context.Context` it receives already carries one (attached by an earlier `Start` call further up this same causal chain, or by `logging.WithTraceID`), it reuses that same ID; otherwise it mints a fresh one (`github.com/google/uuid`). The resolved ID is recorded as every request log line's `trace_id` field.

This is what lets a long-lived WebSocket connection's many separate per-message request logs - each its own independent `Start`/`FinishRequestLog` pair, per the rule above - still be correlated as belonging to one connection: `wsObservability`'s own connection-level `Start` call mints the ID once, and every `Start` call the handler makes afterward (the join handshake's own log, each message's own log, the deferred close log) inherits it from the context `wsObservability` already threaded through, without any of that code needing to know trace IDs exist at all.

Today, in a single deployed process, a trace ID provides no correlation value beyond what a single flushed log line already gives on its own - there is only one service, so there is nothing to correlate a request's log lines across. Its value is deliberately forward-looking: once this system splits into multiple separately-deployed services, `logging.WithTraceID` is the mechanism an inbound request (carrying a trace ID a caller already generated) uses to make this process's own logs join that same correlation, rather than requiring a redesign of `logging` at that point.
