...
Status: ACTIVE (WORK-0029 DONE, independent review APPROVED 2026-09-25 - see "Current Work" below)
...
- **WORK-0029** (Session-Owned Output/Value Schema, decouple `game/session` from `engine`) - **DONE (2026-09-25)**. `sessionlifecycle` now has its own closed `Value`/`Output` interfaces (one concrete type per variant/kind, mirroring `engine`'s own 11 `Value` variants and the 4 `Output` kinds `Outputs` already carries) and a new `outputs.go` mapping `engine.Output`/`engine.Value` into them via `(*Manager).mapOutputs` (recipient actor-id -> `UserUUID` resolution uses `Manager`'s own repository access, a new `internal/repo.FindActorsByIDs`) - see the WORK's own "Design Decisions"/"Implementation Report"/"Completion Record" sections. Independent review APPROVED, two NON_BLOCKING findings fixed same-session (test naming/assertion precision only). `game/CURRENT_STATE.md`/`game/docs/FLOWS.md` synchronized.
...
| — | WORK-0029 — Session-Owned Output/Value Schema (decouple `game/session` from `engine`; the actual mapping a play-half consumer reads from) | DONE |
...
24 WORK total: 9 DONE, 0 IMPLEMENTING, 0 READY, 1 DRAFT, 14 PLANNED.
...
| Presentation state | **DONE** - returned by `Manager.Start`/`AnswerInteraction` in memory, as `sessionlifecycle`'s own exported `Output`/`Value` schema (WORK-0029), not `engine.Output`/`engine.Value`; not yet fanned out to clients | WORK-0006 (DONE), WORK-0029 (DONE); client delivery owned by a not-yet-drafted play-half WORK following WORK-0020 |
| UI effects | **DONE** - returned by `Manager.Start`/`AnswerInteraction` in memory, same session-owned schema; not yet fanned out to clients | WORK-0006 (DONE), WORK-0029 (DONE); client delivery same as above |
