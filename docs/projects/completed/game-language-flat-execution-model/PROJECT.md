...
Status: COMPLETED (all five WORK items DONE; Completion Criteria met; moved to `docs/projects/completed/`, human-confirmed 2026-09-24)
Created: 2026-09-24
Last updated: 2026-09-24 (moved active -> completed, human-confirmed. Prior update, same day: WORK-0028 independent review APPROVED after one NON_BLOCKING fix, closed DONE - all five of this Project's WORK items are now DONE, Completion Criteria met. Prior update, same day: WORK-0028 drafted after a post-closure audit found `sessionlifecycle` still implements its own step-chaining/replay loops on top of `engineservice`, the same class of caller-facing leak GAME-ADR-0026 removed for addressing but never applied to execution - see GAME-ADR-0027)
...
Extended (2026-09-24, WORK-0028, DONE): also implemented `game/docs/decisions/GAME-ADR-0027-engine-owned-turn-execution-and-replay.md` (ACCEPTED) - moved the RuntimeTurn step-chain-draining loop and the GAME-ADR-0024 replay loop, both previously implemented by `sessionlifecycle` itself on top of `engineservice`'s single-signal `Step` primitive, inside `engineservice`. Session Runtime no longer holds, constructs, or reads an `engine.Snapshot` and no longer reimplements any part of the engine's own execution model.
...
| 5 | WORK-0028 — Engine-Owned Turn Execution (Replay and Step-Chain Draining) | DONE |
...
- **WORK-0028** (DONE) moved `sessionlifecycle`'s own step-chaining/replay loops inside `engineservice` (GAME-ADR-0027). It did not depend on WORK-0024–0027's own addressing changes (a different axis - execution/replay ownership, not addressing), but touched the exact same `interaction_capture.go`/`step_answer_interaction.go`/`step_start.go`/`replay.go` files WORK-0027 had just rewritten, and the exact same files `session-runtime-v1`'s WORK-0006 (still DRAFT, not yet resumed as of this WORK's closure) is about to build new Effect/Presentation-Output-handling code against. Its own recommendation - land before WORK-0006 resumes, to avoid building against a `captureInteractions`/Output-handling surface about to change shape again - is now satisfied: `captureInteractions` already takes `[]engine.Output` directly, so WORK-0006 can resume against a settled surface.
...
4. ~~WORK-0028 is DONE, or explicitly moved out of scope with human confirmation - `sessionlifecycle` never holds/constructs/reads an `engine.Snapshot` and never reimplements the engine's own step-chaining/replay mechanics, per GAME-ADR-0027.~~ **Met (2026-09-24)** - independent review APPROVED (one NON_BLOCKING finding, fixed); `sessionlifecycle` confirmed to hold no `engine.Snapshot` anywhere in production code.

All four criteria are met. Human-confirmed 2026-09-24; this Project moved `docs/projects/active/` -> `docs/projects/completed/` (see "Tracked Follow-Ups" above for standalone items intentionally left for later, separate work - none blocked this Project's own completion).
