...
`api/session` is the session workflow's route group, and today the only one: it hosts the WebSocket transport adapter for the Live Session Coordinator (`../play/`, see `../play/README.md`) - one HTTP upgrade handshake and one read-pump/write-pump goroutine pair per connection, decoding/encoding wire JSON and forwarding decoded client commands to `play`. It holds no Session/connection registry and no business state of its own, and depends only on `play`'s exported API - never on any `game` package.

`Create` (`POST /sessions`) stays ordinary HTTP request/response - it happens before any Participant or connection exists, so there is nothing yet to pair it with. `Join` is *not* a separate HTTP call: `GET /ws` takes `join_code`/`user_uuid`/`display_name`/`idempotency_key`, performs Join, and only upgrades and binds the connection when the outcome means the caller is now an active Participant. A decline or error never upgrades at all - it is reported over plain HTTP instead. This is deliberate: a client that could call HTTP Join successfully without ever separately opening a WebSocket connection would be a persisted Participant the Coordinator had never bound anything for, silently missing whatever the first `Start` delivers - a frontend should never need to know that "join a session" is secretly two independently-failable calls it must both remember to make. Once joined-and-connected, `Start`/`AnswerInteraction` and their resulting fan-out ride that same already-open connection for the rest of its life.
